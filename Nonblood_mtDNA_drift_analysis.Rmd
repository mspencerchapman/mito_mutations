...
figures_dir=notebook_plots_dir("Nonblood_mtDNA_drift_analysis") #see save_plots in config.R
rebuttal_figs_dir=paste0(root_dir,"/plots/additional_plots/") #plots that do not appear in the manuscript figures
plots_dir=notebook_plots_dir("Nonblood_mtDNA_drift_analysis")
...
```{r fig.height=30,fig.width=10.5}
...
all_VAF_distribution_plots=Map(mat=VAF_distribution_mats,dataset=all_cohorts,function(mat,dataset) {
  ...
  save_plot(filename=paste0(plots_dir,"VAF_dist_plot2_",dataset,".pdf"),VAF_dist_plot,width=2.5,height=(0.6*nrow(mat)))
...
```{r fig.height=27,fig.width=10.5}
...
save_plot(filename=paste0(plots_dir,"combined_VAF_distirbution_plot.pdf"),combined_VAF_dist_plot,width=7,height=12)
```
...
save_plot(filename=paste0(plots_dir,"blood_vs_lymph_VAF_dist.pdf"),blood_vs_lymph_VAF_distribution,width=3.3,height=2.5)
```
...
```{r fig.width=22.5,fig.height=4.5}
...
save_plot(plot = homo_muts_per_sample,filename = paste0(plots_dir,"homo_muts_per_sample.pdf"),width=15,height=3)
```
...
save_plot(plot = HSC_vs_lymph_mean_homo_burden,filename = paste0(plots_dir,"HSC_vs_lymph_mean_homo_burden.pdf"),width=3,height=2)
...
save_plot(plot = Mean_vs_Variance_plot,filename = paste0(plots_dir,"homo_muts_mean_vs_variance.pdf"),width=3,height=2)
```
...
save_plot(plot = data_vs_regression,filename = paste0(plots_dir,"data_vs_regression.pdf"),width=3,height=2)
```
...
save_plot(plot = homo_muts_glm_coefficients,filename = paste0(plots_dir,"homo_muts_glm_coefficients.pdf"),width=2,height=2.2)
```
...
save_plot(plot = data_vs_regression_lymph_subdivided,filename = paste0(plots_dir,"data_vs_regression_lymph_subdivided.pdf"),width=3,height=2)
```

Visualize the coefficients for these different lymphocyte types.

```{r fig.width=3.8,fig.height=3.3}
...
save_plot(plot = blood_vs_lymph_subdivided_homo_muts_glm_coefficients,filename = paste0(plots_dir,"blood_vs_lymph_subdivided_homo_muts_glm_coefficients.pdf"),width=2,height=2.2)
```
