...
starting_vafs=c(0.8,0.2)
starting_genotypes=c("wt","a")

fisher_wright_generation=function(starting_vafs,
                             starting_genotypes,
                             generation_time,
                             population_size=600,
                             total_time){
  curr_vafs<-starting_vafs
  total_generations=round(total_time/generation_time)
  for(i in 1:total_generations) {
    curr_vafs<-table(sample(starting_genotypes,size = population_size,replace=T,prob = curr_vafs))/population_size #This is equivalent to sampling with replacement
