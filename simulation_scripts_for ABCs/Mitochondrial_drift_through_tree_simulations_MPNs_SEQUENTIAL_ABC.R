...
for(j in 1:6) {
  ...
  if(file.exists(mut_RDS_file)&!force_rerun){
    ...
    if(file.exists(mut_sim_file)){
      ...
      if(j==1) {
        #If it is the first mutation of the sequential ABC, set the prior from the initial prior parameters
        #run with fixed population size of 600 -> makes it easier to hone in on the correct drift rate
        prior_table<-data.frame(starting_vaf=runif(n=iter),mito_CN=rep(600,iter),log_generation_time=runif(n=iter,min=-1,max=2.7))%>%
          dplyr::mutate(generation_time=10^log_generation_time)
        
      } else {
        
        posterior_table_file=paste0("posterior_table_",abc_muts$mut[j-1],".Rds")
        prior_table<-readRDS(posterior_table_file)
        prior_table<-prior_table%>%
          dplyr::mutate(log_generation_time=sample(log_generation_time,size=iter,replace=T))%>% #Sample the log of the generation time from the previous posterior
          dplyr::mutate(starting_vaf=runif(n=iter),mito_CN=rep(600,iter))%>% #Reset the prior of starting vafs as this is not a shared parameter between mutations
          dplyr::mutate(generation_time=10^log_generation_time)
      }
      ...
      sim_out_list<-lapply(1:iter,function(i) {
        ...
        mito_cn=prior_table$mito_cn[i]
        log_generation_time=prior_table$log_generation_time[i]
