...
figures_dir=notebook_plots_dir("mtDNA_mutations_comparator_tissues") #see save_plots in config.R
...
```{r mean_coverage,fig.width=7,fig.height=2.5}
name_conversion_vec=c("Normal blood","Lymphoid","MPN","Colon","IBD-affected\nColon","MUTYH-mutant\nColon","Endometrium","Bronchial\nepithelium")
names(name_conversion_vec)=c("blood","lymph","NW","HL","SO","PR","LM","KY")
tissue_cols<-c("#96e97c","#fd81c8","#145a6a","#65e6f9","#781486","#5f70cc","#aea2eb","#1d6d1f")
names(tissue_cols)<-name_conversion_vec
...
```{r median_coverage,fig.width=3,fig.height=2.5}
...
```{r uniformity,fig.width=3,fig.height=2.5}
...
temp=Map(dataset_mito_data=all_mito_datasets[all_cohorts_plus_CML_no_lymph],dataset=all_cohorts_plus_CML_no_lymph,function(dataset_mito_data,dataset) {
  ...
  cat(dataset,sep="\n")
  CN_correlating_muts=dataset_mito_data[[1]]$CN_correlating_muts
  figures_dir=paste0(plots_dir,dataset,"/")
  ...
  temp=Map(list=dataset_mito_data,exp_ID=names(dataset_mito_data),function(list,exp_ID){
    ...
    if(is.null(list$shared_muts_df)) {
      save_pdf(file = paste0(figures_dir,exp_ID,"_shared_muts.pdf"),width = 7,height=4)
  ...
  temp=Map(list=dataset_mito_data,exp_ID=names(dataset_mito_data),function(list,exp_ID){
    ...
    if(is.null(list$shared_muts_df)) {
      save_pdf(file = paste0(figures_dir,exp_ID,"_raw_shared_muts.pdf"),width = 7,height=4)
      list[[tree_to_show]]=plot_tree(tree = list[[tree_to_show]],cex.label = 0,plot_axis=T,vspace.reserve = 1.1,title=paste0(exp_ID," (",Age," years old) : Shared mutations (VAF>",100*lowest_VAF_to_show,"%)"))
      dev.off()
      stop(return(NULL))
    }
  ...
  temp=Map(list=dataset_mito_data,exp_ID=names(dataset_mito_data),function(list,exp_ID){
    ...
    par(mfrow=c(1,1))
    save_pdf(file = paste0(figures_dir,exp_ID,"_all_muts_filt.pdf"),width = 7,height=8)
  ...
  temp=Map(list=dataset_mito_data,exp_ID=names(dataset_mito_data),function(list,exp_ID){
    ...
    if(length(plot_muts)==0) {
      save_pdf(file = paste0(figures_dir,exp_ID,"_germline_muts.pdf"),width = 7,height=4)
    ...
    par(mfrow=c(1,1))
    save_pdf(file = paste0(figures_dir,exp_ID,"_germline_muts.pdf"),width = 7,height=8)
...
```{r clonal_marking_plots,fig.width=3,fig.height=2.5}
dataset="CML"
...
```{r clonal_marking_plots2,fig.width=5,fig.height=2.5}
...
```{r copy_number_analysis_ridge_plot,fig.width=3,fig.height=2}
...
```{r copy_number_by_age,fig.width=3,fig.height=2}
...
```{r mean_cn_by_tissue_plot,fig.width=3,fig.height=2}
...
```{r mean_cn_regression_create_df,fig.width=3,fig.height=2}
...
```{r mean_cn_regression,fig.width=3,fig.height=2}
...
```{r model_comparison_AIC,fig.width=3,fig.height=2}
...
```{r model_smoking,fig.width=3,fig.height=2}
...
```{r lmer2_visualization_log,fig.width=3,fig.height=3}
...
```{r lmer2_visualization,fig.width=3,fig.height=2}
...
```{r mut_burden_stats,fig.width=3,fig.height=2}
mutCN_cutoff=25 #If the mitochondrial copy number is over 25, retain mutation even if is in the "CN correlating muts" list
vaf_cut_off<-0.03
rho_cut_off<-0
...
```{r sum_of_vaf_data,fig.width=3,fig.height=2}
...
```{r mutation_rate_by_tissue_prep_data,fig.width=3,fig.height=2}
...
```{r mutation_rate_by_tissue_lmer,fig.width=3,fig.height=2}
...
```{r mutation_rate_by_tissue_lmer2,fig.width=3,fig.height=2}
...
```{r mutation_rate_by_tissue_and_age_lmer_plots,fig.width=3,fig.height=2.5}
...
```{r mutation_rate_by_tissue_and_age_lmer_plots2,fig.width=7,fig.height=2.5}
...
```{r nuclear_genetic_code,fig.width=3,fig.height=2,warning=FALSE,message=FALSE}
...
```{r consequence_change_plot,fig.width=3,fig.height=3}
...
```{r consequence_change_summary,fig.width=3,fig.height=2}
...
```{r consequence_change_plot2,fig.width=3,fig.height=2}
...
```{r consequence_change2,fig.width=3,fig.height=2,message=FALSE}
impact_comparison_df%>%
  filter(tissue!="blood")%>%
  dplyr::select(mut_ref,"new_annotation"=impact,"original_annotation"=impact_NUC)%>%
  tidyr::gather(-mut_ref,key="annotation",value="impact")%>%
...
```{r consequence_change3,fig.width=3,fig.height=2}
...
```{r mut_type_per_gene,fig.width=3,fig.height=2}
...
save_plot(filename = paste0(rebuttal_figs_dir,"mutation_type_by_gene_CROSSTISSUE.pdf"),mutation_type_by_gene,width=3.5,height=2.5)
```

Now summarise the coding mutation consequences per tissue (across genes).

```{r mut_type_by_tissue,fig.width=3,fig.height=2}
...
save_plot(filename = paste0(rebuttal_figs_dir,"mutation_type_by_tissue_CROSSTISSUE.pdf"),mutation_type_by_tissue,width=3.5,height=2)
```
...
```{r combine_epithelial_tissues,fig.width=3,fig.height=2,warning=F,message=F}
...
```{r ecdf_epithelial_tissues,fig.width=3,fig.height=2.5,warning=F,message=F}
...
save_plot(filename = paste0(rebuttal_figs_dir,"mutation_type_ecdf_plot_EPITHELIAL.pdf"),mutation_type_ecdf_plot,width=2.5,height=2)
```

Now, SPLIT THE EPITHELIAL TISSUE analysis at the dichotomous vaf threshold from blood (20%)

```{r epithelial_tissues_dnds_dichot_vaf,fig.width=3,fig.height=2,warning=F,message=F}
...
```{r epithelial_tissues_dnds_by_vaf_and_gene,fig.width=3,fig.height=2}
...
```{r dnds_by_tissue,fig.width=3,fig.height=2,warning=F,message=F}
...
```{r gene_level_dnds_by_tissue,fig.width=5,fig.height=2.5}
qval_cutoff=0.05
...
```{r dnds_by_tissue_by_vaf,fig.width=3,fig.height=2}
...
```{r global_dnds_by_tissue_by_vaf2,fig.width=7,fig.height=2.5,warning=F,message=F}
...
```{r dnds_by_tissue_by_vaf_MPN_and_lymph_only,fig.width=3,fig.height=2}
max_dnds_value<-5
...
```{r dnds_by_tissue_by_vaf_nonblood_only,fig.width=3,fig.height=2}
max_dnds_value<-2.5
...
```{r gene_level_dnds_by_tissue_by_vaf,fig.width=4,fig.height=10}
gene_order=c(paste0("MT-ND",1:6),"MT-ND4L","MT-CYB",paste0("MT-CO",1:3),"MT-ATP6","MT-ATP8")
gene_order<-gene_order[gene_order%in%target_genes]
...
```{r dnds_heatmap_by_tissue_and_vaf_by_gene_blood_and_lymph_only,fig.width=5,fig.height=5}
...
```{r dnds_by_dichotomous_vaf_by_tissue,fig.width=3,fig.height=3,warning=F,message=F}
...
```{r dnds_by_dichotomous_vaf_by_tissue_split,fig.width=5,fig.height=2.5}
max_dnds_value<-2.8
...
```{r gene_dnds_by_tissue_and_dichotomous_vaf,fig.width=4,fig.height=10}
gene_order=c(paste0("MT-ND",1:6),"MT-ND4L","MT-CYB",paste0("MT-CO",1:3),"MT-ATP6","MT-ATP8")
gene_order<-gene_order[gene_order%in%target_genes]
...
```{r MPN_selection_by_driver_WT,fig.width=3,fig.height=2,eval=F}
...
```{r MPN_selection_by_driver_WT2,fig.width=3,fig.height=2,eval=FALSE}
...
```{r heteroplasmic_oocyte_mutations,fig.width=3,fig.height=2}
...
```{r heteroplasmic_oocyte_mutations2,fig.width=3,fig.height=2,message=F,warning=F}
library(dndscv)
library(mitovizR)
...
```{r compare_heteroplasmic_oocyte_to_all_somatic,fig.width=3,fig.height=2}
...
```{r compare_heteroplasmic_oocyte_to_all_somatic2,fig.width=5,fig.height=5}
