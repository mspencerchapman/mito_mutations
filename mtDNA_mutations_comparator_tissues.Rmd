---
title: "mtDNA Mutational Analysis - comparator tissues (primarily the 'non-blood' dataset)"
author: "Mike Spencer Chapman"
date: "`r Sys.Date()`"
output: html_document
---

This markdown goes through all the analysis on the comparator cross-tissue dataset from the manuscript "Mitochondrial mutation, drift and selection during human development and ageing".
...
cran_packages=c("devtools","ape","stringr","dplyr","tidyr","ggplot2","gridExtra","phylosignal","ggpubr","VGAM","usethis","RColorBrewer","MCMCglmm","phangorn","spam","dichromat","ggridges","forcats","lmerTest","tidyverse","readr","patchwork","BuenColors","Seurat")
...
if(!require("treemut", character.only=T,quietly = T, warn.conflicts = F)){
  remotes::install_github("nangalialab/treemut")
...
if(!require("mitovizR", character.only=T,quietly = T, warn.conflicts = F)){
  remotes::install_github("robertopreste/mitovizR", build_vignettes = TRUE)
...
plots_dir=notebook_plots_dir("mtDNA_mutations_comparator_tissues")
...
Set some other variables and colour schemes used in the script.
e.g. defining which initials are used as keys for which dataset.
...
## ASSESS MITOCHONDRIAL COVERAGE & UNIFORMITY

First just look at the mean mitochondrial coverage per sample as determined by bedtools.
Plot this as tissue-specific histograms.

```{r mean_coverage,fig.width=7,fig.height=4.5}
name_conversion_vec=c("Normal blood","Lymphoid","MPN","Colon","IBD-affected\nColon","MUTYH-mutant\nColon","Endometrium","Bronchial\nepithelium")
names(name_conversion_vec)=c("blood","lymph","NW","HL","SO","PR","LM","KY")
tissue_cols<-c("#96e97c","#fd81c8","#145a6a","#65e6f9","#781486","#5f70cc","#aea2eb","#1d6d1f")
names(tissue_cols)<-name_conversion_vec
...
coverage.plot<-mito_cn%>%
  ...
  geom_histogram(col="black",linewidth=0.2,bins=30)+
  scale_fill_manual(values=tissue_cols)+
  scale_x_log10(labels=scales::label_comma())+
  facet_wrap(~Tissue,scales = "free_y",nrow=2)+
  theme_bw()+
  my_markdown_theme+
  theme(axis.text.x = element_text(angle=90),strip.text.x = element_text(size = 12,margin = margin(1,0,1,0, "mm")))+
  labs(x="Mean mitochondrial DNA coverage",y="Count")
coverage.plot
save_plot(filename=paste0(plots_dir,"coverage_by_tissue_plot.pdf"),coverage.plot,width=7,height=2.5)
```

Now use the median coverage across the mitochondrial, a more standard figure utilised in these studies.

```{r median_coverage,fig.width=7,fig.height=5.8}
...
save_plot(filename=paste0(plots_dir,"median.coverage.plot.pdf"),median.coverage.plot,width=3.3,height=2.5)
```
...
```{r uniformity,fig.width=7,fig.height=5.8}
...
uniformity_plot
save_plot(filename=paste0(plots_dir,"uniformity_plot.pdf"),uniformity_plot,width=3.3,height=2.5)
```


```{r coverage_summary_stats1}
...
```{r coverage_summary_stats_overall}
#Overall
mito_cn%>%
  filter(Tissue%in%translate$al_ref)%>%
  right_join(sample_summary_info,by=c("Sample"="SampleID"))%>%
...
This is already in the saved version, and this takes a while so don't need to re-run unless you want to change some parameters.

```{r shared_muts,eval=FALSE}
...
The lymphocyte colonies don't have a phylogeny, therefore can't meaningfully plot these overlying a phylogeny. Therefore, remove these from the dataset here.

For the remaining individuals, here we do several plots:

(1) shared mutations - but only those that pass shearwater
(2) shared mutations - in more "raw form", including those that don't pass shearwater
(3) all mutations that pass shearwater in at least one sample: BIG HEATMAPS. This is the highest granularity visualization - show down to 0.001 VAF.

These are saved as as separate files for viewing as not practical within the R markdown.

```{r shared_muts_plotting, include=F}
all_cohorts_plus_CML_no_lymph<-all_cohorts_plus_CML[!all_cohorts_plus_CML=="lymph"]

temp=Map(dataset_mito_data=all_mito_datasets[all_cohorts_plus_CML_no_lymph],dataset=all_cohorts_plus_CML_no_lymph,function(dataset_mito_data,dataset) {
  
  cat(dataset,sep="\n")
  CN_correlating_muts=dataset_mito_data[[1]]$CN_correlating_muts
  #Per-dataset subfolder for the heatmaps below; save_pdf() creates it when
  #save_plots is on, so there is no need to make it here.
  dataset_plots_dir=paste0(plots_dir,dataset,"/")
  
  ### Visualize the shared mutations, showing only VAFs > 3% ----
  ...
  temp=Map(list=dataset_mito_data,exp_ID=names(dataset_mito_data),function(list,exp_ID){
    ...
    if(is.null(list$shared_muts_df)) {
      save_pdf(file = paste0(dataset_plots_dir,exp_ID,"_shared_muts.pdf"),width = 7,height=4)
    ...
    if(length(plot_muts)>=2) {
      plot_muts.clustered<-hclust(stats::dist(vaf.mtx[plot_muts,]))
    } else {
  ...
  temp=Map(list=dataset_mito_data,exp_ID=names(dataset_mito_data),function(list,exp_ID){
    ...
    if(is.null(list$shared_muts_df)) {
      save_pdf(file = paste0(dataset_plots_dir,exp_ID,"_raw_shared_muts.pdf"),width = 7,height=4)
      list[[tree_to_show]]=plot_tree(tree = list[[tree_to_show]],cex.label = 0,plot_axis=T,vspace.reserve = 1.1,title=paste0(exp_ID," (",Age," years old) : Shared mutations (VAF>",100*lowest_VAF_to_show,"%)"))
      dev.off()
      stop(return(NULL))
    }
    ...
    if(length(plot_muts)>=2) {
      plot_muts.clustered<-hclust(stats::dist((vaf.mtx^(1/4))[plot_muts,]))
    } else {
      plot_muts.clustered=list(order=1)
    }
  ...
  temp=Map(list=dataset_mito_data,exp_ID=names(dataset_mito_data),function(list,exp_ID){
    ...
    par(mfrow=c(1,1))
    save_pdf(file = paste0(dataset_plots_dir,exp_ID,"_all_muts_filt.pdf"),width = 7,height=8)
  ...
  temp=Map(list=dataset_mito_data,exp_ID=names(dataset_mito_data),function(list,exp_ID){
    ...
    if(length(plot_muts)==0) {
      save_pdf(file = paste0(dataset_plots_dir,exp_ID,"_germline_muts.pdf"),width = 7,height=4)
    ...
    if(length(plot_muts)>=2) {plot_muts.clustered<-hclust(stats::dist((vaf.mtx^(1/4))[plot_muts,]))} else {plot_muts.clustered=list(order=1)}
    
    par(mfrow=c(1,1))
    save_pdf(file = paste0(dataset_plots_dir,exp_ID,"_germline_muts.pdf"),width = 7,height=8)
...
```{r clonal_marking_plots,fig.width=7,fig.height=5.8}
dataset="CML"
...
expanded.clades.plot
save_plot(filename=paste0(plots_dir,dataset,"_expanded_clades_plot.pdf"),expanded.clades.plot,width=4,height=2.5)
```
#### Get info on 'marker mutations'
For the previously selected dataset, find the best "marker mutations" and visualize this.

First, there is this slightly complex snippet of code to pull out any mutations that are:
 (1) shared by ≥2 clone samples
 (2) are not present in a large fraction of samples outside the clone (must be <20%)

```{r clonal_marking_plots2,fig.width=7,fig.height=4.5}
...
expanded.clades.marking.plot
save_plot(filename=paste0(plots_dir,"expanded_clades_marking_plot_",dataset,".pdf"),expanded.clades.marking.plot,width=7,height=2.3)
```


```{r clonal_marking_plots3}
...
Analyse the mitochondrial DNA copy number, by individual.

Here we have a boxplot of the mtDNA copy number for each individual as a boxplot. The boxplots are grouped by their tissue of origin. Note that the y-axis is logarithmic as there is huge between sample variability (both within and between tissues).

```{r copy_number_analysis_boxplot,fig.width=10,fig.height=4}
...
cn_by_tissue_plot<-mito_cn%>%
  ...
  theme_classic()+
  my_markdown_theme+
  #Tissue panels are sized by sample count (space="free"), so the narrow ones cannot
  #fit a horizontal label - rotate the strips so the label length is limited by panel
  #height rather than width, as done for the per-donor facets elsewhere in this notebook.
...
cn_by_tissue_plot
save_plot(filename=paste0(plots_dir,"cn_by_tissue_plot.pdf"),cn_by_tissue_plot,width=10,height=4)
```

Given that tissue type is a major driver of mtDNA copy number, summarise this by group all samples from a given tissue (aggregating across individuals) into a single ridge plot.

```{r copy_number_analysis_ridge_plot,fig.width=7,fig.height=4.7}
...
mitoCN_ridges_plot
save_plot(filename=paste0(plots_dir,"mitoCN_ridges_plot.pdf"),mitoCN_ridges_plot,width=3,height=2)
```

Within each tissue, is there a difference in copy number through life? No - nothing convincing.

```{r copy_number_by_age,fig.width=7,fig.height=4.7}
...
median_cn_by_age_plot
save_plot(filename=paste0(plots_dir,"median_cn_by_age_plot.pdf"),median_cn_by_age_plot,width=3.3,height=2)
```

Mean copy number by tissue.
Given the log-normal distribution, perform the means on the log-transformed data then transform back to absolute numbers.

```{r mean_cn_by_tissue_plot,fig.width=7,fig.height=4.7}
...
mean_cn_plot
save_plot(filename=paste0(plots_dir,"mean_cn_plot.pdf"),mean_cn_plot,width=3,height=2)
```

Create a dataframe to use with the linear mixed effects model.

```{r mean_cn_regression_create_df,fig.width=7,fig.height=4.7}
...
```{r mean_cn_regression,fig.width=7,fig.height=4.7}
...
AIC analysis shows that adding in the age variable (model 2) actually increases the AIC. Suggesting that the model is slightly worse.

```{r model_comparison_AIC,fig.width=7,fig.height=4.7}
...
LMER MODEL 3: See if bronchial CN correlates with smoking status?

No significant correlation in our data. However, this may be confounded by the in vitro colony formation or the fact that these are purified epithelial cell.

```{r model_smoking,fig.width=7,fig.height=4.7}
...
To visualize the different tissue effects on mtDNA copy number, show the tissue co-efficients from the LME model.
This initial visualization shows the log values.

```{r lmer2_visualization_log,fig.width=7,fig.height=7}
...
tissue_lmer_coefs_plot<-as.data.frame(summary(mito_cn.lmer)$coefficients)%>%
  ...
  theme(legend.position="none")
tissue_lmer_coefs_plot
save_plot(filename=paste0(plots_dir,"tissue_lmer_coefs_plot.pdf"),tissue_lmer_coefs_plot,width=3,height=2)
```
Here, convert back to the absolute values.  This is essentially like doing a regression on the geometric mean mtDNA value.

```{r lmer2_visualization,fig.width=7,fig.height=4.7}
...
tissue_lmer_exp_coefs_plot<-as.data.frame(summary(mito_cn.lmer)$coefficients)%>%
  ...
  theme(legend.position="none")
tissue_lmer_exp_coefs_plot
save_plot(filename=paste0(plots_dir,"tissue_lmer_exp_coefs_plot.pdf"),tissue_lmer_exp_coefs_plot,width=3,height=2)
```

# MUTATIONAL PROFILES - 96-profile plots


```{r trinuc_profiles1,fig.width=7,fig.height=4}
trinucleotide_plot(mutations = all_df_tidy[all_cohorts]%>%
                   ...
                   file_name = plot_file(paste0(plots_dir,"Mutational_signature_nonblood_all.pdf")))
```


```{r trinuc_profiles2,fig.width=7,fig.height=4}
#Low-VAF profile: cut at 0.1, matching the output file name and the published
#Extended Data Fig. 3d ("<10%"). This filter previously read vaf<0.05.
trinucleotide_plot(mutations = all_df_tidy[all_cohorts]%>%
                     dplyr::bind_rows()%>%
                     filter(Sample!="global"&
                              !mut_ref%in%exclude_muts&
                   ...
                   analysis_region = "all_mtDNA",
                   analysis_type = "obs_exp",
                   file_name = plot_file(paste0(plots_dir,"Mutational_signature_non_blood_under0.1.pdf")))
```
...
```{r mut_burden_stats,fig.width=7,fig.height=4.7}
mutCN_cutoff=25 #If the mitochondrial copy number is over 25, retain mutation even if is in the "CN correlating muts" list
vaf_cut_off<-0.03
rho_cut_off<-0
...
### Save 'prop of samples with mut' plots for each dataset

```{r prop_of_samples_with_mut,fig.width=7,fig.height=12}
...
plot_list=Map(dataset_mito_data=all_mito_datasets[all_cohorts],df_tidy=all_df_tidy[all_cohorts],this_dataset=all_cohorts, function(dataset_mito_data,df_tidy,this_dataset) {
  ...
  prop.of.samples.with.mut<-samples_with_mut_df%>%
    ...
    labs(x="VAF cut-off threshold",
         ...
         col="Individual",title = name_conversion_vec[this_dataset])+
    my_markdown_theme+
    theme(legend.key.height = unit(2.8,"mm"),legend.title = element_text(size = 14))
  save_plot(filename=paste0(plots_dir,"Sample_proportions_with_mut_",this_dataset,".pdf"),prop.of.samples.with.mut,width=3.5,height=2)
  return(prop.of.samples.with.mut)
})
...
```{r sum_of_vaf_data,fig.width=7,fig.height=4.7}
...
```{r sum_of_vaf_data_plots,fig.width=7,fig.height=14}
dont_show_medians_tissues<-c("KY","LM","SO") #Too many individuals to print median values on the plot (gets too busy)

plot_list=lapply(all_cohorts,function(this_dataset) {
  ...
  if(this_dataset%in%dont_show_medians_tissues) {
    sum_of_vaf_plot_log<-all_sum_of_vaf_df%>%
      ...
      theme_classic()+
      my_markdown_theme+
      theme(axis.text.x=element_blank(),axis.ticks.x=element_blank(),strip.text.x = element_text(size = 12))+
    ...
    save_plot(filename = paste0(plots_dir,"sum_of_vaf_log_",this_dataset,".pdf"),plot=sum_of_vaf_plot_log,height=2,width=7)
  return(sum_of_vaf_plot_log)
})
...
```{r mutation_rate_by_tissue_prep_data,fig.width=7,fig.height=4.7}
...
#Prepare the data for LMER models, excluding individuals with very few samples
...
```
First just do LMER models by dataset.
Do 3 models of increasing complexity: 
(1) null model - constant burden across tissues/ age
(2) age term
(3) age and tissue interaction terms (i.e. tissue-specific mutation accumulation rates)
(4) age and tissue with full interaction (i.e. tissue-specific intercepts and accumulation)

By AIC, by far the best model is the one with only an age term, and no tissue effect.

```{r mutation_rate_by_tissue_lmer,fig.width=7,fig.height=4.7}
...
Now categorise by tissue type, not just dataset as some studies actually include the same tissues. Could this be masking the tissue effect?
No, the original model with only an age term is still the best model by AIC.

```{r mutation_rate_by_tissue_lmer2,fig.width=7,fig.height=4.7}

#Add a "tissue_cat" variable which groups together Colon samples and the haematopoietic samples
...
#Run the regression with this as the predictor variable
...
Plot the results of the model with tissue-specific age coefficients.
While this wasn't the "best model" this is still helpful to give a hint of any tissue-specific differences in accumulation rate, and how big these might be.

```{r mutation_rate_by_tissue_and_age_lmer_plots,fig.width=7,fig.height=5.8}
...
```{r mutation_rate_by_tissue_and_age_lmer_plots2,fig.width=7,fig.height=4.5}
...
mutburden_by_age_and_tissue_plot<-ggplot() +
  ...
  theme(legend.position="none")
mutburden_by_age_and_tissue_plot
save_plot(filename = paste0(plots_dir,"mutburden_by_age_and_tissue_plot.pdf"),mutburden_by_age_and_tissue_plot,width=7,height=2)
```

## SELECTION ANALYSIS

```{r annotate_mutations,message=FALSE,warning=FALSE}
library(dndscv)
...
```{r nuclear_genetic_code,fig.width=7,fig.height=4.7,warning=FALSE,message=FALSE}
...
Here we show how the incorrect annotation changes  the numbers of mutations with different consequences (i.e. truncating vs missense vs synonymous)

```{r consequence_change_plot,fig.width=7,fig.height=7}
impact_comparison_df<-left_join(complete.annotated.mutation.table,
          ...
          by = join_by(tissue, sampleID, mut_ref, codonsub))%>%
...
save_plot(filename = paste0(plots_dir,"consequence_change_plot_CROSSTISSUE.pdf"),consequence_change_plot,width=3.5,height=2.5)
```


```{r consequence_change_summary,fig.width=7,fig.height=4.7}
...
```{r consequence_change_plot2,fig.width=7,fig.height=4.7}
...
save_plot(filename = paste0(plots_dir,"total_numbers_old_vs_new_plot_CROSSTISSUE.pdf"),total_numbers_old_vs_new_plot,width=3,height=2.5)
```


```{r consequence_change2,fig.width=7,fig.height=4.7,message=FALSE}
impact_comparison_df%>%
  filter(tissue!="blood")%>%
  dplyr::select(mut_ref,"new_annotation"=impact,"original_annotation"=impact_NUC)%>%
  tidyr::gather(-mut_ref,key="annotation",value="impact")%>%
...
```{r consequence_change3,fig.width=7,fig.height=4.7}
...
simple_impact_comparison_df%>%
  ...
  left_join(new_muts,by="impact")%>%left_join(lost_muts,by="impact")%>%
...
### Summarise coding mutation consequences

First summarise the coding mutation consequences per gene.
This is looking across all the comparator datasets (see we exclude only the original "blood" mutations that we analysed separately).

```{r mut_type_per_gene,fig.width=7,fig.height=4.7}
...
save_plot(filename = paste0(plots_dir,"mutation_type_by_gene_CROSSTISSUE.pdf"),mutation_type_by_gene,width=3.5,height=2.5)
```

Now summarise the coding mutation consequences per tissue (across genes).

```{r mut_type_by_tissue,fig.width=7,fig.height=4.7}
...
mutation_type_by_tissue
save_plot(filename = paste0(plots_dir,"mutation_type_by_tissue_CROSSTISSUE.pdf"),mutation_type_by_tissue,width=3.5,height=2)
```
### Combine epithelial tissues

Given the different cellular dynamics of epithelial tissues, it seems plausible that mitochondrial dynamics and selection may be different across epithelial tissues. Therefore, to increase power to detect this, combine results across colon, bronchial epithlium and endometrium.

```{r combine_epithelial_tissues,fig.width=7,fig.height=4.7,warning=F,message=F}
...
```{r ecdf_epithelial_tissues,fig.width=7,fig.height=5.8,warning=F,message=F}
...
save_plot(filename = paste0(plots_dir,"mutation_type_ecdf_plot_EPITHELIAL.pdf"),mutation_type_ecdf_plot,width=2.5,height=2)
```

Now, SPLIT THE EPITHELIAL TISSUE analysis at the dichotomous vaf threshold from blood (20%)

```{r epithelial_tissues_dnds_dichot_vaf,fig.width=7,fig.height=4.7,warning=F,message=F}
...
dnds_global_by_dichotomous_vaf_epithelial_plot<-globaldnds_by_vaf_epithelial%>%
  ...
  theme(axis.text.x = element_text(angle=0),
        legend.position="none")
dnds_global_by_dichotomous_vaf_epithelial_plot
save_plot(filename = paste0(plots_dir,"dnds_global_by_dichotomous_vaf_epithelial_plot.pdf"),dnds_global_by_dichotomous_vaf_epithelial_plot,width=3,height=2)
```


```{r epithelial_tissues_dnds_by_vaf_and_gene,fig.width=7,fig.height=4.7}
...
dnds_heatmap_epithelial_and_vaf_by_gene
save_plot(filename = paste0(plots_dir,"dnds_heatmap_epithelial_and_vaf_by_gene.pdf"),dnds_heatmap_epithelial_and_vaf_by_gene,width=2.5,height=2)
```
### Generate global dnds by each individual tissue dataset

```{r dnds_by_tissue,fig.width=7,fig.height=4.7,warning=F,message=F}
...
global_dnds_nonblood
save_plot(filename = paste0(plots_dir,"global_dnds_CROSSTISSUE.pdf"),global_dnds_nonblood,width=2,height=2)
```

### Get gene-level dNdS info by tissue

```{r gene_level_dnds_by_tissue,fig.width=7,fig.height=4.5}
qval_cutoff=0.05
...
dnds_heatmap_by_gene_by_tissue
save_plot(filename = paste0(plots_dir,"dnds_heatmap_by_gene_by_tissue.pdf"),dnds_heatmap_by_gene_by_tissue,width=4,height=2.5)
```

### Selection split by heteroplasmy

Enhance the tidy dataframe with 'VAF bin' info

```{r dnds_by_tissue_by_vaf,fig.width=7,fig.height=4.7}
...
```{r global_dnds_by_tissue_by_vaf2,fig.width=7,fig.height=4.5,warning=F,message=F}
...
global_dnds_by_tissue_and_vaf_plot
save_plot(filename = paste0(plots_dir,"global_dnds_by_tissue_and_vaf_plot_CROSSTISSUE.pdf"),global_dnds_by_tissue_and_vaf_plot,width=7,height=2)
```


```{r dnds_by_tissue_by_vaf_MPN_and_lymph_only,fig.width=7,fig.height=4.7}
max_dnds_value<-5
global_dnds_by_vaf_MPN_and_lymph_onlyplot<-globaldnds_res_by_tissue_and_vaf%>%
  ...
  theme(legend.position="none",
        ...
        axis.title.x=element_blank(),
        strip.text.y = element_text(angle = 0))
global_dnds_by_vaf_MPN_and_lymph_onlyplot
save_plot(filename = paste0(plots_dir,"global_dnds_by_vaf_MPN_and_lymph_onlyplot.pdf"),global_dnds_by_vaf_MPN_and_lymph_onlyplot,width=2,height=2)
```


```{r dnds_by_tissue_by_vaf_nonblood_only,fig.width=7,fig.height=4.7}
max_dnds_value<-2.5
global_dnds_by_vaf_normal_nonblood_onlyplot<-globaldnds_res_by_tissue_and_vaf%>%
  ...
  theme(legend.position="none",
        ...
        axis.title.x=element_blank(),
        strip.text.y = element_text(angle = 0))
global_dnds_by_vaf_normal_nonblood_onlyplot
save_plot(filename = paste0(plots_dir,"global_dnds_by_vaf_normal_nonblood_onlyplot.pdf"),global_dnds_by_vaf_normal_nonblood_onlyplot,width=2.5,height=2)
```


```{r gene_level_dnds_by_tissue_by_vaf,fig.width=7,fig.height=17.5}
gene_order=c(paste0("MT-ND",1:6),"MT-ND4L","MT-CYB",paste0("MT-CO",1:3),"MT-ATP6","MT-ATP8")
gene_order<-gene_order[gene_order%in%target_genes]
...
selcv_res_by_tissue_and_vaf<-Map(list1=dndscv_by_tissue_and_vaf,this_tissue=names(dndscv_by_tissue_and_vaf),function(list1,this_tissue) {
  ...
  Map(dndsout=list1,VAF_bin=new_VAF_groups[-1],function(dndsout,VAF_bin) {
    temp<-dndsout$sel_cv%>%
      dplyr::select(gene_name,wmis_cv,wnon_cv,qmis_cv,qtrunc_cv)
    ...
    mutate(VAF_group=factor(VAF_group,levels=VAF_groups$labels[-1]),tissue=this_tissue)
...
dnds_heatmap_by_tissue_and_vaf_by_gene<-selcv_res_by_tissue_and_vaf%>%
  mutate(dnds_if_signif=ifelse(qval<qval_cutoff,paste0(sprintf(dNdS, fmt = '%#.1f'),"*"),""),
         ...
         gene_name=factor(gene_name,levels=rev(gene_order)),
         tissue=factor(name_conversion_vec[tissue],levels=name_conversion_vec))%>%
  filter(VAF_group%in%VAF_groups$labels[-1] & tissue!="Blood")%>%
  ggplot(aes(y=gene_name,x=VAF_group,fill=dNdS,label=dnds_if_signif))+
  geom_tile()+
  ...
  scale_fill_gradientn(colors=RColorBrewer::brewer.pal(n=11,name = "RdYlGn"))+
  labs(x="Variant allele fraction",y="")
dnds_heatmap_by_tissue_and_vaf_by_gene
save_plot(filename = paste0(plots_dir,"dnds_heatmap_by_tissue_and_vaf_by_gene.pdf"),dnds_heatmap_by_tissue_and_vaf_by_gene,width=4,height=6.5)
```

Split into two groups so that can fit in the figure.

```{r split_into_groups,fig.width=10,fig.height=5}
## GROUP 1
...
dnds_heatmap_by_tissue_and_vaf_by_gene_group1<-selcv_res_by_tissue_and_vaf%>%
  mutate(dnds_if_signif=ifelse(qval<qval_cutoff,paste0(sprintf(dNdS, fmt = '%#.1f'),"*"),""),
         ...
         gene_name=factor(gene_name,levels=rev(gene_order)),
         tissue=factor(name_conversion_vec[tissue],levels=name_conversion_vec))%>%
  filter(VAF_group%in%VAF_groups$labels[-1] & tissue%in%group1_tissues)%>%
  ggplot(aes(y=gene_name,x=VAF_group,fill=dNdS,label=dnds_if_signif))+
  geom_tile()+
  facet_grid(tissue~type)+
  geom_text(col="white",size=1.5)+
...
dnds_heatmap_by_tissue_and_vaf_by_gene_group2<-selcv_res_by_tissue_and_vaf%>%
  mutate(dnds_if_signif=ifelse(qval<qval_cutoff,paste0(sprintf(dNdS, fmt = '%#.1f'),"*"),""),
         ...
         gene_name=factor(gene_name,levels=rev(gene_order)),
         tissue=factor(name_conversion_vec[tissue],levels=name_conversion_vec))%>%
  filter(VAF_group%in%VAF_groups$labels[-1] & tissue%in%group2_tissues)%>%
  ggplot(aes(y=gene_name,x=VAF_group,fill=dNdS,label=dnds_if_signif))+
  geom_tile()+
  facet_grid(tissue~type)+
  geom_text(col="white",size=1.5)+
...
save_plot(filename = paste0(plots_dir,"dnds_heatmap_by_tissue_and_vaf_by_gene_group1.pdf"),dnds_heatmap_by_tissue_and_vaf_by_gene_group1,width=3.5,height=4)
save_plot(filename = paste0(plots_dir,"dnds_heatmap_by_tissue_and_vaf_by_gene_group2.pdf"),dnds_heatmap_by_tissue_and_vaf_by_gene_group2,width=3.5,height=3.2)

dnds_heatmap_by_tissue_and_vaf_by_gene_group1+dnds_heatmap_by_tissue_and_vaf_by_gene_group2
```


```{r dnds_heatmap_by_tissue_and_vaf_by_gene_blood_and_lymph_only,fig.width=7,fig.height=7}
...
dnds_heatmap_by_tissue_and_vaf_by_gene_blood_and_lymph_only<-selcv_res_by_tissue_and_vaf%>%
  mutate(dnds_if_signif=ifelse(qval<qval_cutoff,paste0(sprintf(dNdS, fmt = '%#.1f'),"*"),""),
         ...
         gene_name=factor(gene_name,levels=rev(gene_order)),
         tissue=factor(name_conversion_vec[tissue],levels=name_conversion_vec))%>%
  filter(VAF_group%in%VAF_groups$labels[-1] & tissue%in%c("Lymphoid","Normal blood"))%>%
  ggplot(aes(y=gene_name,x=VAF_group,fill=dNdS,label=dnds_if_signif))+
  geom_tile()+
  facet_grid(tissue~type)+
  geom_text(col="white",size=1.5)+
...
dnds_heatmap_by_tissue_and_vaf_by_gene_blood_and_lymph_only
save_plot(filename = paste0(plots_dir,"dnds_heatmap_by_tissue_and_vaf_by_gene_blood_and_lymph_only.pdf"),dnds_heatmap_by_tissue_and_vaf_by_gene_blood_and_lymph_only,width=4,height=4)
```

### Split by dichotomous VAF group (less or greater than 20% only) 

```{r dnds_by_dichotomous_vaf_by_tissue,fig.width=7,fig.height=7,warning=F,message=F}
...
global_dnds_by_tissue_and_dichotomous_vaf_plot
save_plot(filename = paste0(plots_dir,"global_dnds_by_tissue_and_dichotomous_vaf_plot_CROSSTISSUE.pdf"),global_dnds_by_tissue_and_dichotomous_vaf_plot,width=2.5,height=2.5)
```


```{r dnds_by_dichotomous_vaf_by_tissue_split,fig.width=7,fig.height=4.5}
max_dnds_value<-2.8
global_dnds_by_dichotomous_vaf_MPN_and_lymph_onlyplot<-globaldnds_res_by_tissue_and_dichotomous_vaf%>%
  filter(VAF_group%in%VAF_groups$labels[2:nrow(VAF_groups)] & name%in%stats_to_include)%>%
  mutate(cihigh=ifelse(cihigh>max_dnds_value,max_dnds_value,cihigh),
         VAF_group=factor(VAF_group,levels=VAF_groups$labels[2:nrow(VAF_groups)]),
         tissue=factor(name_conversion_vec[tissue],levels=name_conversion_vec))%>%
  filter(tissue%in%c("Lymphoid","MPN"))%>%
  ggplot(aes(x=VAF_group, y = mle, ymin = cilow, ymax = cihigh, color = name, shape = name)) +
  geom_hline(yintercept=1, linetype='dashed', col = 'black', linewidth = 0.5) +
...
save_plot(filename = paste0(plots_dir,"global_dnds_by_dichotomous_vaf_MPN_and_lymph_onlyplot.pdf"),global_dnds_by_dichotomous_vaf_MPN_and_lymph_onlyplot,width=2,height=2)


global_dnds_by_dichotomous_vaf_normal_nonblood_onlyplot<-globaldnds_res_by_tissue_and_dichotomous_vaf%>%
  filter(VAF_group%in%VAF_groups$labels[-1] & name%in%stats_to_include)%>%
  mutate(cihigh=ifelse(cihigh>max_dnds_value,max_dnds_value,cihigh),
         VAF_group=factor(VAF_group,levels=VAF_groups$labels[-1]),
         tissue=factor(name_conversion_vec[tissue],levels=name_conversion_vec))%>%
...
save_plot(filename = paste0(plots_dir,"global_dnds_by_dichotomous_vaf_normal_nonblood_onlyplot.pdf"),global_dnds_by_dichotomous_vaf_normal_nonblood_onlyplot,width=2.5,height=2)

global_dnds_by_dichotomous_vaf_MPN_and_lymph_onlyplot+global_dnds_by_dichotomous_vaf_normal_nonblood_onlyplot
```


```{r gene_dnds_by_tissue_and_dichotomous_vaf,fig.width=7,fig.height=17.5}
gene_order=c(paste0("MT-ND",1:6),"MT-ND4L","MT-CYB",paste0("MT-CO",1:3),"MT-ATP6","MT-ATP8")
gene_order<-gene_order[gene_order%in%target_genes]

selcv_res_by_tissue_and_dichotomous_vaf<-Map(list1=dndscv_by_tissue_and_vaf,this_tissue=names(dndscv_by_tissue_and_vaf),function(list1,this_tissue) {
  
  Map(dndsout=list1,VAF_bin=new_VAF_groups[2:length(new_VAF_groups)],function(dndsout,VAF_bin) {
    temp<-dndsout$sel_cv%>%
      dplyr::select(gene_name,wmis_cv,wnon_cv,qmis_cv,qtrunc_cv)
    ...
    mutate(VAF_group=factor(VAF_group,levels=VAF_groups$labels[2:length(new_VAF_groups)]),tissue=this_tissue)
  
})%>%dplyr::bind_rows()

dnds_heatmap_by_tissue_and_dichotomous_vaf_by_gene<-selcv_res_by_tissue_and_dichotomous_vaf%>%
  mutate(dnds_if_signif=ifelse(qval<qval_cutoff,paste0(sprintf(dNdS, fmt = '%#.1f'),"*"),""),
         dNdS=ifelse(dNdS>2,2,dNdS),
         gene_name=factor(gene_name,levels=rev(gene_order)),
         tissue=factor(name_conversion_vec[tissue],levels=name_conversion_vec))%>%
  filter(VAF_group%in%VAF_groups$labels[2:nrow(VAF_groups)] & tissue!="Normal blood")%>%
  ggplot(aes(y=gene_name,x=VAF_group,fill=dNdS,label=dnds_if_signif))+
  geom_tile()+
  facet_grid(tissue~type)+
  geom_text(col="white",size=1.5)+
...
dnds_heatmap_by_tissue_and_dichotomous_vaf_by_gene
save_plot(filename = paste0(plots_dir,"dnds_heatmap_by_tissue_and_dichotomous_vaf_by_gene.pdf"),dnds_heatmap_by_tissue_and_dichotomous_vaf_by_gene,width=4,height=6.5)
```

## MPN analysis, test global dNdS by driver clone/ wild-type

```{r MPN_selection_by_driver_WT,fig.width=7,fig.height=4.7,eval=F,message=FALSE,warning=FALSE}
...
dndscv_by_vaf_and_mut_status<-lapply(list(mut=all_mut,wt=all_wt),function(sample_set) {
  dndscv_by_vaf<-lapply(new_VAF_groups[-1],function(VAF_bin) {
    cat(VAF_bin,sep="\n")
...
MPN_global_dnds_by_mut_status_plot
save_plot(filename = paste0(plots_dir,"MPN_global_dnds_by_mut_status_plot.pdf"),MPN_global_dnds_by_mut_status_plot,width=3.5,height=2.5)
```

# MPN analysis, test gene-level dNdS by driver clone/ wild-type and VAF

```{r MPN_selection_by_driver_WT2,fig.width=7,fig.height=4.7,eval=FALSE}
...
MPN_dnds_heatmap_by_mut_status_by_gene<-selcv_res_by_mut_status%>%
  ...
  scale_fill_gradientn(colors=RColorBrewer::brewer.pal(n=11,name = "RdYlGn"))+
  labs(x="Variant allele fraction",y="")
MPN_dnds_heatmap_by_mut_status_by_gene
save_plot(filename = paste0(plots_dir,"MPN_dnds_heatmap_by_mut_status_by_gene.pdf"),MPN_dnds_heatmap_by_mut_status_by_gene,width=3.5,height=3.5)
```
...
This is based on the principle that mutations present across multiple unrelated clones (till you go back to development),are most likely to have been present in the oocyte and drifted up in unrelated lineages.
The alternative - possible for some of these - are that these are examples of independent acquisition.

```{r heteroplasmic_oocyte_mutations,fig.width=7,fig.height=4.7}
...
prop_of_samples_with_ho_mut
save_plot(filename = paste0(plots_dir,"prop_of_samples_with_ho_mut.pdf"),prop_of_samples_with_ho_mut,width=2.5,height=1.8)
```


```{r heteroplasmic_oocyte_mutations2,fig.width=7,fig.height=4.7,message=F,warning=F}
library(dndscv)
library(mitovizR)
...
n_of_het_oocyte_muts_plot
save_plot(filename = paste0(plots_dir,"n_of_het_oocyte_muts_plot.pdf"),n_of_het_oocyte_muts_plot,width=2,height=1.8)
```

Compare mutation distribution with full somatic mutation set

```{r compare_heteroplasmic_oocyte_to_all_somatic,fig.width=7,fig.height=4.7}
...
Mut_cat_comparison
save_plot(filename = paste0(plots_dir,"Mut_cat_comparison.pdf"),Mut_cat_comparison,width=3,height=2.5)
```

Save plot of het oocyte mutations by genome position.

```{r compare_heteroplasmic_oocyte_to_all_somatic2,fig.width=7,fig.height=7}
...
mtDNA_mut_pos
save_plot(filename = paste0(plots_dir,"mtDNA_mut_pos.pdf"),mtDNA_mut_pos,width=5,height=5)
```

## Do example clone inferences on an individuals with heteroplasmic oocyte muts

This is to demonstrate how inferring clones without considering the possibility of heteroplasmic oocyte mutations can completely misinform the inference. This is not always the case, but is an issue in selected cases.

```{r inferring_clones_from_het_oocyte,fig.width=3,fig.height=2}
library(Seurat)
...
output.dir <- paste0(root_dir,"/data/mito_mut_clones/nonblood/")
...
for (i in 1:nrow(patient.info)){
  ...
  # save heatmaps
  save_pdf(paste0(plots_dir, patient.tmp, "_mito_mutation_heatmap.pdf"), width=15, height=8)
  print(hm) 
  dev.off()
  ...
  if(length(groups) > 2){
    ...
    mito.hc <- hclust(stats::dist(lsa::cosine((matty))))
    
    save_pdf(paste0(plots_dir, patient.tmp, "_hierarchical_tree.pdf"), width = 5, height = 5)
    print(plot(mito.hc))
    dev.off()
...
```{r inferring_clones_from_het_oocyte2,fig.width=10,fig.height=6}
...
for(j in 1:nrow(patient.info)) {
  ...
  this.patient.info<-all_mito_datasets[[patient.dataset]][[patient.tmp]]
  exp_clones<-read.delim(paste0(output.dir, "/", patient.tmp, "_mtdna_clone_assignment.txt"))
  ...
  #Now save it
  save_pdf(file=paste0(plots_dir,"Inferred_clones_",patient.tmp,".pdf"),width = 7,height=3)
  par(mfrow=c(1,1))
  this.patient.info$tree=plot_tree(tree = this.patient.info$tree,cex.label = 0)
  clone_hm<-matrix(NA,nrow=1,ncol=length(this.patient.info$tree$tip.label),dimnames = list("Clones",this.patient.info$tree$tip.label))
  for(i in 1:nrow(exp_clones)){
