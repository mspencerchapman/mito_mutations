...
real_mut_distribution_plot<-real_muts_dist_df%>%
  ...
  theme_bw()+
  my_markdown_theme+
  theme(axis.text.x = element_text(size = 6,angle=90),strip.text.x = element_text(size = 12))+
...
real_mut_distribution_plot+my_markdown_theme+theme(strip.text.x = element_text(size = 12),axis.text.x = element_text(size = 6,angle=90))
```
...
real_mut_distribution_plot_normalized<-left_join(real_muts_dist_df,nsamp_df)%>%
  ...
  #facet_grid(rows=vars(factor(exp_ID,levels=ref_df$Sample[order(ref_df$Age)])),scales="free")+
  facet_wrap(~factor(exp_ID,levels=ref_df$Sample[order(ref_df$Age)]),scales="fixed",dir="v",ncol=6)+
  labs(x="VAF range",y="Number of N1 mutations\ndetected per sample")

real_mut_distribution_plot_normalized+my_markdown_theme+theme(strip.text.x = element_text(size = 12),axis.text.x = element_text(size = 6,angle=90))
save_plot(filename=paste0(plots_dir,"real_mut_distribution_plot.pdf"),real_mut_distribution_plot_normalized,width=7,height=2.5)
```
...
Visualize the distribution at a selection of "generations" to get a sense of the evolution of the distribution through time. Note that the colouring of a mutation tells you when in life it was actually acquired. Red mutations were acquired very early in life, whereas blue mutations were acquired late. This demonstrates that while low VAF mutations were invariably acquired recently, high VAF mutations are usually acquired decades ago, but there aren't very many until late in life.
...
modelled_VAF_dist<-gens_record%>%
  ...
  theme_bw()+
  theme(legend.position = "none",axis.text.x = element_text(size = 6,angle=90),strip.text.x = element_text(size = 12))+
  scale_fill_manual(values = colorRampPalette(RColorBrewer::brewer.pal(8,"Spectral"))(1200))+
  labs(x="VAF group",y="Count")+
  my_markdown_theme

modelled_VAF_dist+my_markdown_theme+theme(strip.text.x = element_text(size = 12),axis.text.x = element_text(size = 6,angle=90))
save_plot(filename=paste0(plots_dir,"Modelled_VAF_dist.pdf"),modelled_VAF_dist,width = 6,height=2)
```
...
acquisition_time_by_VAF<-gens_record%>%
  ...
  theme_classic()+
  my_markdown_theme+
  theme(axis.text.x=element_text(angle=90),strip.text.x = element_text(size = 14,margin = unit(c(1,0,1,0),"mm")))+
  labs(x="Time of mutation acquisition\n(WF Generations)",y="Density")

acquisition_time_by_VAF+my_markdown_theme
save_plot(filename = paste0(plots_dir,"Acquisition_time_by_VAF.pdf"),acquisition_time_by_VAF,width=7,height=2.5)
```
...
acquisition_time_by_VAF_ridges+my_markdown_theme
save_plot(filename = paste0(plots_dir,"acquisition_time_by_VAF_ridges.pdf"),acquisition_time_by_VAF_ridges,width=3.3,height=2.5)
```
...
drift_with_age_animation<-gens_record_summary%>%
  ...
  theme_bw()+
  my_markdown_theme+
  theme(title = element_text(size = 14),legend.position = "none",axis.text.x = element_text(angle=90,size=10),axis.title.x=element_text(size = 14),axis.title.y=element_text(size = 14))+
...
drift_with_age_animation+my_markdown_theme
anim <- gganimate::animate(drift_with_age_animation,renderer = gifski_renderer())
save_anim(filename=paste0(plots_dir,"driftwithage.gif"),animation = anim)
```
...
scale = (length(lut)-1)
save_pdf(paste0(plots_dir,"mito_muts_by_gen_scale.pdf"),height=6,width = 3)
...
abc_res_plot_mut_rate<-abc_res_df%>%
  ...
  geom_errorbar(width=0.3,alpha=0.5,linewidth=0.7)+
  theme_classic()+
...
abc_res_plot_mut_rate+my_markdown_theme
save_plot(filename=paste0(plots_dir,"abc_res_plot_mut_rate.pdf"),abc_res_plot_mut_rate,width=1.75,height=2)
```
...
abc_res_plot_total_generations<-abc_res_df%>%
  ...
  geom_jitter(aes(col=exp_ID),alpha=0.1,size=0.3,width=0.1,height = 0)+
...
abc_res_plot_total_generations+my_markdown_theme
save_plot(filename=paste0(plots_dir,"abc_res_plot_total_generations.pdf"),abc_res_plot_total_generations,width=2,height=2)
```
...
real_syn_mut_distribution_plot_normalized<-left_join(syn_muts_distribution_df,nsamp_df)%>%
  ...
  #facet_grid(rows=vars(factor(exp_ID,levels=ref_df$Sample[order(ref_df$Age)])),scales="free")+
  facet_wrap(~factor(exp_ID,levels=ref_df$Sample[order(ref_df$Age)]),scales="fixed",dir="v",ncol=6)+
  labs(x="VAF range",y="Number of mutations not affecting\ncoding sequence detected per sample")

real_syn_mut_distribution_plot_normalized+my_markdown_theme+theme(axis.text.x = element_text(size = 6,angle=90),strip.text.x = element_text(size = 12))

save_plot(filename=paste0(plots_dir,"synonymous_noncoding_distribution_plot.pdf"),real_mut_distribution_plot_normalized,width=7,height=2.5)
```
...
abc_synres_plot_mut_rate+my_markdown_theme

save_plot(filename=paste0(plots_dir,"abc_synres_plot_mut_rate.pdf"),abc_synres_plot_mut_rate,width=1.75,height=2)
```
...
```{r fig.width=7,fig.height=5.6}
#95% CI of the fitted trend - see lmer_confidence_band() for what the band covers
gens_by_age_syn_band<-lmer_confidence_band(lme.gens_by_age.syn,abc_syn_res_df,predictor="Age")

abc_synres_plot_total_generations<-abc_syn_res_df%>%
  dplyr::filter(!exp_ID%in%c("8pcw","18pcw"))%>%
  mutate(exp_ID=factor(exp_ID,levels=ref_df$Sample[order(ref_df$Age)]))%>%
  ggplot(aes(x=Age,y=total_generations))+
  geom_ribbon(data=gens_by_age_syn_band,aes(x=Age,ymin=lower,ymax=upper),
              inherit.aes=FALSE,fill="grey50",alpha=0.25)+
  geom_jitter(aes(col=exp_ID),alpha=0.1,size=0.3,height=0,width=0.1)+
  scale_y_continuous(limits=c(0,1700))+
...
abc_synres_plot_total_generations+geom_point(aes(col=exp_ID),alpha=0.1,size=0.5)+my_markdown_theme+guides(colour=guide_legend(override.aes = list(alpha=1,size=1)))+
  theme(legend.box.spacing = unit(0,"mm"),
...
save_plot(filename=paste0(plots_dir,"abc_synres_plot_total_generations.pdf"),abc_synres_plot_total_generations,width=2,height=2)
```
