...
setwd("/Users/al28/Mounts/cancer/mtDNA/Analysis/shearwater_exome/")

tissues <- list.files()

for(tiss in 1:length(tissues)){
  setwd(paste0("/Users/al28/Mounts/cancer/mtDNA/Analysis/shearwater_exome/",tissues[tiss]))
  ...
  for(x in 1:length(input_files)){
    ...
    germline_muts <- mutations[which(mutations$tum_globalvaf >= 0.5),]
