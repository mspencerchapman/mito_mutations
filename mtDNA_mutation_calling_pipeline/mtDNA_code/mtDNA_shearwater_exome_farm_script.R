...
exome_coverage <- read.table(file = "exome_coverage_pileup_and_bedtools_annotated.csv", sep = ",", header = T, stringsAsFactors = F)
manual_exclusion <- read.table(file = "excluded_samples_exome.csv", sep = ",", header = T, stringsAsFactors = F)

excluded_samples_automatic <- exome_coverage[which(exome_coverage$verify_bam_id_freemix >= 0.01 | exome_coverage$haplocheck_contamination >= 0.01),c(1,3)]
excluded_samples_manual <- manual_exclusion[,c(1,3)]
colnames(excluded_samples_manual) <- colnames(excluded_samples_automatic)

excluded_samples <- rbind(excluded_samples_automatic,excluded_samples_manual)
excluded_samples <- excluded_samples[which(!(duplicated(paste(excluded_samples$Tissue,excluded_samples$Sample,sep = "_")))),]
...
normcounts_obj <- list()
normcounts_obj$counts <- readRDS(file = "shearwater_background_exome.Rds")
norm_total <- apply(normcounts_obj$counts[, , 1:6] + normcounts_obj$counts[, , 7:12], c(2, 3), sum)
norm_total <- norm_total / rowSums(norm_total)

normal_rho_panel <- read.table("/lustre/scratch125/casm/team268im/al28/mtDNA/shearwater_normal_panel_exome_rho_mu_calculation.tsv", sep = "\t", stringsAsFactors = F, header = T)
normal_rho_panel$rho[which(normal_rho_panel$rho <= 2e-04)] <- 2e-04

tissue_samples <- as.character(exome_coverage$Sample[which(exome_coverage$Tissue == tissue)])
tissue_samples <- tissue_samples[which(!(tissue_samples %in% excluded_samples$Sample[which(excluded_samples$Tissue == tissue)]))]
...
write.table(x = mutations_allsamples, file = paste0("Analysis/shearwater_exome/",tissue,"/",donor,"_shearwater_mutations.tsv"), sep = "\t", quote = F, col.names = T, row.names = F)
