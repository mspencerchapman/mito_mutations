...
mtdna_ref <- readLines(con = "/Users/al28/Mounts/cancer/bed_ref/rCRS.fasta", warn = F)
...
for(i in 1:length(tissue_list)){
  ...
  for(j in 1:length(donors)){
    ...
    ggplot(germline_data) +
      ...
      theme_bw()
    ggsave(paste0("/Users/al28/Mounts/cancer/mtDNA/Analysis/global_vaf_heatmaps/",tissue_list[i],"/",donors[j],"_global_vaf_histogram.pdf"), width = 15, height = 5)
    ...
    write.table(x = sample_vaf_array, file = paste0("/Users/al28/Mounts/cancer/mtDNA/Analysis/global_vaf_heatmaps/",tissue_list[i],"/",donors[j],"_donor_vaf_table.tsv"), sep = "\t", row.names = T, col.names = T, quote = F)
    ...
    ggsave(paste0("/Users/al28/Mounts/cancer/mtDNA/Analysis/global_vaf_heatmaps/",tissue_list[i],"/",donors[j],"_vaf_heatmap_global_vaf_threshold.pdf"), width = 40, height = 40)
