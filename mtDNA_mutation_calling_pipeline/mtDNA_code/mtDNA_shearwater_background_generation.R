...
shearwater_panel_samples <- read.table(file = "shearwater_normal_panel", sep = "\t", header = T, stringsAsFactors = F)

tissue_list <- list.files("Analysis/global_vaf_heatmaps/")

for(z in 1:length(tissue_list)){
  if(file.exists(paste0("Analysis/global_vaf_heatmaps/",tissue_list[z],"/",tissue_list[z],"_combined_germline_indels_merged.tsv"))){
    if(!(exists("all_tissue_germline"))){
      all_tissue_germline <- read.table(file = paste0("Analysis/global_vaf_heatmaps/",tissue_list[z],"/",tissue_list[z],"_combined_germline_indels_merged.tsv"), header = T, stringsAsFactors = F, sep = "\t")
    } else{
      all_tissue_germline <- rbind(all_tissue_germline, read.table(file = paste0("Analysis/global_vaf_heatmaps/",tissue_list[z],"/",tissue_list[z],"_combined_germline_indels_merged.tsv"), header = T, stringsAsFactors = F, sep = "\t"))
    }
  }
}
...
saveRDS(shearwater_background, file = "shearwater_background.Rds")