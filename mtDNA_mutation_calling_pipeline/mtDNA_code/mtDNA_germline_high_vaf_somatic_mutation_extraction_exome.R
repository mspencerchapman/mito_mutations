setwd("/Users/al28/Mounts/cancer/mtDNA/Analysis/global_vaf_heatmaps_exome/")

excluded_samples <- read.table("/Users/al28/Mounts/cancer/mtDNA/excluded_samples_exome.csv", sep = ",", stringsAsFactors = F, header = T)
  ...
  for(y in 1:length(donors)){
    ...
    if(nrow(high_vaf_calls) > 0){
      ...
      for(j in 1:ncol(high_vaf_calls)){
        if(length(which(high_vaf_calls[,j] > 0.9)) >= (floor(0.9 * nrow(high_vaf_calls)))){
