...
for(i in 1:length(tissue_list)){
  ...
  if(tissue_list[i] == "blood"){
    ...
    # metadata <- read.table("/lustre/scratch125/casm/team268im/al28/mtDNA/project_metadata/colon_ibd.txt", sep = "\t", stringsAsFactors = F, header = T)
  ...
  } else if(tissue_list[i] == "blood_MPN") {
  ...
  for(j in 1:length(donors)){
    if(tissue_list[i] == "blood"){
      ...
      donor_samples <- tissue_samples[grep(paste0("^",donors[j]),tissue_samples)]
    } else {
      donor_samples <- tissue_samples[which(substr(tissue_samples,1,7) == donors[j])]
    }
    ...
    threshold <- max(c((mean(rowSums(donor_cov)) * 0.25) / (length(donor_samples)), 100)) / mean(rowSums(donor_cov))
    ...
    ggplot(germline_data) +
      ...
      theme_bw()
    ggsave(paste0("/lustre/scratch125/casm/team268im/al28/mtDNA/Analysis/global_vaf_heatmaps/",tissue_list[i],"/",donors[j],"_global_vaf_histogram.pdf"), width = 15, height = 5)
    ...
    write.table(x = sample_vaf_array, file = paste0("/lustre/scratch125/casm/team268im/al28/mtDNA/Analysis/global_vaf_heatmaps/",tissue_list[i],"/",donors[j],"_donor_vaf_table.tsv"), sep = "\t", row.names = T, col.names = T, quote = F)
    ...
    ggsave(paste0("/lustre/scratch125/casm/team268im/al28/mtDNA/Analysis/global_vaf_heatmaps/",tissue_list[i],"/",donors[j],"_vaf_heatmap_global_vaf_threshold.pdf"), width = 40, height = 40)
