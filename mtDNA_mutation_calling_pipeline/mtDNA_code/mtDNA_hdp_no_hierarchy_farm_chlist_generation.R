# for i in *; do cd $i; for j in {1..10}; do bsub -q normal -R 'select[mem>=7500] rusage[mem=7500]' -M7500 -e no_hierarchy/chlist_generation_$j.err -o no_hierarchy/chlist_generation_$j.out /software/R-3.6.1/bin/Rscript /lustre/scratch125/casm/team268im/al28/mtDNA/al28_code/mtDNA_hdp_no_hierarchy_farm_chlist_generation.R $j; done; cd ..; done
...
ppindex <- c(0, rep(1,nrow(mut_count)))
cpindex <- c(1, rep(2, nrow(mut_count)))
...
chlist <- hdp_posterior(hdp_activated,
                        ...
                        cpiter=3,
                        seed=iter*1e3) 
saveRDS(chlist, paste0("no_hierarchy/chlist_",iter,".rds"))
