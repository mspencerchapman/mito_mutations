...
setwd("/Users/al28/Mounts/cancer/mtDNA/Analysis/hdp/shearwater_stringent_exclusion/")
...
for(x in 1:length(analysis_list)){
  ...
  pdf(file = "likelihood_plot_tissue_hierarchy.pdf", width = 10, height = 20, useDingbats = F)
  par(mfrow=c(5,2), mar=c(4, 4, 2, 1))
  p1 <- lapply(chains(mut_example_multi), plot_lik, bty="L", start = 1000)
  dev.off()
  
  pdf(file = "cluster_num_plot_tissue_hierarchy.pdf", useDingbats = F)
  par(mfrow=c(5,2), mar=c(4, 4, 2, 1))
  p2 <- lapply(chains(mut_example_multi), plot_numcluster, bty="L")
  dev.off()
  
  pdf(file = "prop_data_assigned_plot_tissue_hierarchy.pdf", useDingbats = F)
  par(mfrow=c(5,2), mar=c(4, 4, 2, 1))
  ...
  pdf(file = "prop_data_items_assigned_plot_tissue_hierarchy.pdf", width = 12, height = 12, useDingbats = F)
  ...
  pdf(file = "sigs_extracted_signatures_plot_tissue_hierarchy.pdf", height = 5, width = 15, useDingbats = F)
  ...
  pdf(file = "signatures_per_sample_plot_tissue_hierarchy_nonsig_not_shown.pdf", useDingbats = F)
  for(i in 1:length(tissues)) {
    if(i == 1) {
      cum_tissues <- 0
    } else {
      cum_tissues <- sum(tissues[1:(i-1)])
    }
    

    dpindices <- 1+length(tissues)+which(sub("_[^_]+$", "", rownames(mut_count)) == names(tissues[i]))
    exposures <- t(dp_distn$mean[dpindices, , drop = FALSE])
    cis <- dp_distn$cred.int[dpindices]
    nonsig <- lapply(cis, function(x) which(x[1, ] == 0))
    for (z in 1:length(nonsig)) {
      exposures[nonsig[[z]], z] <- 0
    }
    inc <- which(rowSums(exposures, na.rm = T) > 0)
    
    if(length(inc != 0)){
    plot_dp_comp_exposure(mut_example_multi, main_text=names(tissues)[i],
                          dpindices=1+length(tissues)+which(sub("_[^_]+$", "", rownames(mut_count)) == names(tissues[i])), 
                          ...
                          incl_nonsig=F,
                          dpnames = paste(sub("^.*_", "", rownames(mut_count))[(cum_tissues + 1):(cum_tissues + tissues[i])]," - ",as.character(as.vector(apply(mut_count, 1, sum)[(cum_tissues + 1):(cum_tissues + tissues[i])]))),
  ...
  pdf(file = "signatures_per_sample_plot_tissue_hierarchy_nonsig_shown.pdf", useDingbats = F)
  for(i in 1:length(tissues)) {
    if(i == 1) {
      cum_tissues <- 0
    } else {
  ...
  mean_sig_assignment <- mean_sig_assignment[-(1:(length(tissues) + 1)),]
