...
for (i in 1:nrow(samples)) {
  if(!(paste0(samples[i,2],"_MT.bam") %in% bam_files)){
    system(sprintf("bsub -q normal -R 'select[mem>=2000] rusage[mem=2000]' -M2000 -e logs/%s.err -o logs/%s.out samtools view -h /nfs/cancer_ref01/nst_links/live/%s/%s/%s.sample.dupmarked.bam MT -o %s_MT.bam -b",
