...
for(i in 1:length(tissue_list)){
  ...
  for(j in 1:length(donors)){
    ...
    system(sprintf("bsub -J %s -q normal -R 'select[mem>=5000] rusage[mem=5000]' -M5000 -e %s.err -o %s.out /software/R-3.6.1/bin/Rscript %s %s %s", job_id, log_path, log_path, shearwater_fun, tissue, donor))
