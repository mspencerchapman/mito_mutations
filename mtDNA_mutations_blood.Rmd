...
cran_packages=c("devtools","ape","stringr","dplyr","tidyr","ggplot2","gridExtra","phylosignal","ggpubr","VGAM","usethis","RColorBrewer","MCMCglmm","phangorn","spam","dichromat","ggridges","forcats","lmerTest","tidyverse","readr")
bioconductor_packages=c("MutationalPatterns","BSgenome","BSgenome.Hsapiens.UCSC.hg19","TxDb.Hsapiens.UCSC.hg19.knownGene","Rsamtools","ggtree")
...
plots_dir=notebook_plots_dir("mtDNA_mutations_blood")
...
exclude_muts=c("MT_302_A_C","MT_311_C_T","MT_567_A_C","MT_574_A_C","MT_16181_A_C","MT_16182_A_C","MT_16183_A_C","MT_16189_T_C") #These are the ones to exclude that particularly affect the endometrial analysis
...
```{r coverage_plot,fig.height=5,fig.width=7}
...
coverage.plot<-mito_cn%>%
  ...
  geom_histogram(fill="lightblue",col="black",linewidth=0.2,bins=30)+
...
coverage.plot
save_plot(filename=paste0(plots_dir,"Coverage_plot.pdf"),coverage.plot,width=4,height=3)
```


```{r median_coverage_plot,fig.height=4.5,fig.width=7}
...
save_plot(filename=paste0(plots_dir,"Coverage_median_plot.pdf"),median.coverage.plot,width=7,height=2)
```

A ridges plot is perhaps better here to compare.
```{r coverage_ridges,fig.height=5.2,fig.width=7}
...
save_plot(filename=paste0(plots_dir,"median.coverage.ridges.plot.pdf"),median.coverage.ridges.plot,width=3.3,height=2.5)
```
...
```{r uniformity_ridges,fig.height=6.1,fig.width=7}
...
save_plot(filename=paste0(plots_dir,"Uniformity_perc80_ridges_plot.pdf"),uniformity_ridges.plot,width=3.3,height=2.5)
```

Print coverage summary statistics
```{r coverage_summary_stats}
mito_cn%>%
  dplyr::filter(Study%in%ref_df$Canapps.project & Sample%in% unlist(lapply(mito_data,function(list) list$tree$tip.label)))%>%
  group_by(exp_ID)%>%
...
```{r mito_cn_pheno_summary_plot,fig.height=5,fig.width=7}
...
mito_cn_by_pheno<-mito_cn%>%
  ...
  theme(#axis.text.x = element_text(angle=90),
        strip.text.x = element_text(size = 14,margin = margin(t=1,r=0,b=1,l=0,unit="mm")),
        legend.key.size = unit(0,"mm"),
        legend.position="right")
mito_cn_by_pheno
save_plot(filename=paste0(plots_dir,"mito_cn_by_pheno.pdf"),mito_cn_by_pheno,width=4,height=2)
```
...
mito_cn_by_pheno_all_HSCprog<-mito_cn%>%
  ...
  my_markdown_theme+
  theme(axis.text.x = element_text(angle=90),strip.text.x = element_text(size = 12,margin = margin(t=1,r=0,b=1,l=0,unit="mm")),legend.position="none",legend.title = element_text(size = 14))
mito_cn_by_pheno_all_HSCprog
save_plot(filename=paste0(plots_dir,"mito_cn_by_pheno_HSCprog.pdf"),mito_cn_by_pheno_all_HSCprog,width=4,height=2)
...
```{r mito_cn_by_sample_dot_plot,fig.height=4.5,fig.width=7}
...
mtDNA.copy.number.logscale<-mito_cn%>%
  ...
  labs(x="Individual",y="Mitochondrial copy number")+
  my_markdown_theme+
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        legend.title = element_text(size = 16),
        legend.key.height = unit(2,"mm"),
        strip.text.x = element_text(size = 7), #narrow space="free" panels truncate larger strip labels
        panel.spacing.x=unit(1, "mm"))+
  guides(colour = guide_legend(override.aes = list(size=2)))

mtDNA.copy.number.logscale

save_plot(filename = paste0(plots_dir,"mtDNA.copy.number.logscale.pdf"),plot=mtDNA.copy.number.logscale,height=2,width=7)
```
...
```{r mito_cn_by_sample_dot_plot_by_phenotype,fig.height=4.5,fig.width=7}
...
save_plot(filename = paste0(plots_dir,"mtDNA.copy.number.logscale.by.phenotype.pdf"),plot=mtDNA.copy.number.logscale.by.phenotype,height=2,width=7)
```
...
save_plot(filename=paste0(plots_dir,"foetal_18pcw_by_cell_type_loc.pdf"),foetal_18pcw_by_cell_type_loc,width=3,height=2)
```
...
median_mtDNA_CN_by_age<-mito_cn%>%
  ...
  geom_smooth(aes(x=Age,y=median),method="lm",inherit.aes = F,col="black",linewidth=0.5)+
  ...
  my_markdown_theme+
  theme(legend.key.height = unit(3,"mm"),legend.title=element_text(size = 14),legend.text=element_text(size = 12))
median_mtDNA_CN_by_age
save_plot(filename = paste0(plots_dir,"median_mtDNA_CN_by_age.pdf"),plot = median_mtDNA_CN_by_age,width=3.5,height=2)
```
...
```{r mtDNA_CN_phylogenetic_signal_pvals,fig.height=5.6,fig.width=7}
...
df_tidy<-dplyr::bind_rows(Map(list=mito_data,exp_ID=names(mito_data),function(list,exp_ID) {
  ...
  #This now only includes cells where the ML_sig is N1
...
Generate a dataframe of mean number of unique mutations per cell. Though not particularly interesting.
...
prop.of.samples.with.mut<-samples_with_mut_df%>%
  ...
  labs(x="Cut-off",
       ...
       col="Individual")+
  my_markdown_theme+
  theme(legend.key.height = unit(3,"mm"),legend.title = element_text(size = 14))

prop.of.samples.with.mut

save_plot(filename=paste0(plots_dir,"Sample_proportions_with_mut.pdf"),prop.of.samples.with.mut,width=3,height=2)
...
Look at the relationship between mean sum of vaf per cell, and the age of the individual.
This has a clear linear relationship.
...
mean_sum_of_vaf_by_age<-sum_of_vaf_summary%>%
  ...
  my_markdown_theme+
  theme(legend.key.height = unit(3,"mm"),legend.title=element_text(size = 14),legend.text=element_text(size = 10))

mean_sum_of_vaf_by_age

save_plot(filename=paste0(plots_dir,"mean_sum_of_vaf_by_age.pdf"),plot=mean_sum_of_vaf_by_age,height=2,width=3)
```
...
sum_of_vaf_plot_log<-sum_of_vaf_df%>%
  ...
  theme_classic()+
  my_markdown_theme+
  theme(axis.text.x=element_blank(),axis.ticks.x=element_blank(),strip.text.x = element_text(size = 7))+ #narrow space="free" panels truncate larger strip labels
  facet_grid(cols=vars(factor(exp_ID,levels=ref_df$Sample[order(ref_df$Age)])),scales = "free",space="free")+
...
save_plot(filename = paste0(plots_dir,"sum_of_vaf_log.pdf"),plot=sum_of_vaf_plot_log,height=2,width=7)
```

Now coloured by cell type, as requested by the reviewer.
...
save_plot(filename = paste0(plots_dir,"sum_of_vaf_log_by_celltype.pdf"),plot=sum_of_vaf_plot_log_by_celltype,height=2.2,width=7)
```

Comparison of mutation burden by cell type (HSCs and progenitor cell types) - violin plot, with overlaid comparisons using the non-parametric Wilcoxon test.
This does not show any significant difference.
...
save_plot(filename = paste0(plots_dir,"sum_of_vaf_by_celltype_comparison.pdf"),plot=sum_of_vaf_by_celltype,height=2.2,width=7)
...
```{r trinuc_plot_all,fig.height=4,fig.width=7}
trinucleotide_plot(mutations = df_tidy%>%
                   ...
                   file_name = plot_file(paste0(plots_dir,"Mutational_signature.pdf")))
```

Now limit to check the 0.5 - 1% VAF mutations.
```{r trinuc_plot_0.5-1,fig.height=4,fig.width=7}
trinucleotide_plot(mutations = df_tidy%>%
                     filter(Sample!="global"&
                              !mut_ref%in%exclude_muts&
                              vaf<0.01 & vaf>0.005)%>%
                   ...
                   analysis_region = "all_mtDNA",
                   analysis_type = "obs_exp",
                   file_name = plot_file(paste0(plots_dir,"Mutational_signature_0.005_0.01.pdf")))
```

Now check the <0.5 VAF mutations.
```{r trinuc_plot_sub0.5,fig.height=4,fig.width=7}
trinucleotide_plot(mutations = df_tidy%>%
                     filter(Sample!="global"&
                              !mut_ref%in%exclude_muts&
                              vaf<0.005)%>%
                   ...
                   analysis_region = "all_mtDNA",
                   analysis_type = "obs_exp",
                   file_name = plot_file(paste0(plots_dir,"Mutational_signature_<0.005.pdf")))
...
```{r mutburden_by_expansion_status}
...
```{r mutburden_by_driverstatus,fig.height=4.5,fig.width=7}
...
save_plot(filename = paste0(plots_dir,"sov_mut_vs_wt.pdf"),sov_mut_vs_wt,width=3.3,height=2.5)
```
...
```{r mutburden_by_driver_status_regression}
...
```{r mutburden_by_clonestatus,fig.height=4.5,fig.width=7}
...
save_plot(filename = paste0(plots_dir,"sov_exp_vs_singleton.pdf"),sov_exp_vs_singleton,width=3.3,height=2.5)
```
...
```{r annotate_variants_with_dndscv,message=FALSE,warning=FALSE}
library(dndscv)
...
```{r nuclear_genetic_code,message=FALSE,warning=FALSE}
...
```{r consequence_change_plot,fig.height=5.2,fig.width=7}
...
save_plot(filename = paste0(plots_dir,"consequence_change_plot.pdf"),consequence_change_plot,width=3.5,height=2.5)
```


```{r compare_misannotated_to_corrected1,fig.height=7,fig.width=7}
...
```{r compare_misannotated_to_corrected2,fig.height=7,fig.width=7}
...
save_plot(filename = paste0(plots_dir,"total_numbers_old_vs_new_plot.pdf"),total_numbers_old_vs_new_plot,width=3,height=2.5)
```


```{r compare_misannotated_to_corrected3,fig.height=7,fig.width=7}
impact_comparison_df%>%
...
```{r compare_misannotated_to_corrected4,,fig.height=7,fig.width=7}
...
```{r mutations_by_type_violin,fig.height=4.2,fig.width=7}
...
```{r heteroplasmy_cum_distribution,fig.height=7,fig.width=7}
...
save_plot(filename = paste0(plots_dir,"mutation_type_ecdf_plot.pdf"),mutation_type_ecdf_plot,width=2.5,height=2.5)
```


```{r mutation_type_by_gene,fig.height=5.8,fig.width=7}
...
save_plot(filename = paste0(plots_dir,"mutation_type_by_gene_blood.pdf"),mutation_type_by_gene,width=3.5,height=2.5)
...
```{r run_dnds_across_tissues,message=FALSE,warning=FALSE}
...
save_plot(filename = paste0(plots_dir,"global_dnds_blood.pdf"),global_dnds_blood,width=1.3,height=2.5)
```


```{r dnds_by_gene_heatmap}
...
save_plot(filename = paste0(plots_dir,"dnds_heatmap_by_gene.pdf"),dnds_heatmap_by_gene,width=1.5,height=2.5)

```

### dndscv - split by heteroplasmy

```{r dnds_by_heteroplasmy,message=FALSE,warning=FALSE}
...
save_plot(filename = paste0(plots_dir,"global_dnds_by_vaf_blood.pdf"),global_dnds_by_vaf_plot,width=1.6,height=2.5)
...
save_plot(filename = paste0(plots_dir,"global_dnds_bytype_by_vaf_plot.pdf"),global_dnds_bytype_by_vaf_plot,width=2.5,height=2.5)
...
save_plot(filename = paste0(plots_dir,"global_dnds_missense_by_vaf_plot.pdf"),global_dnds_missense_by_vaf_plot,width=1.6,height=2.5)
...
save_plot(filename = paste0(plots_dir,"dnds_heatmap_by_vaf_by_gene.pdf"),dnds_heatmap_by_vaf_by_gene,width=2.5,height=2.5)

```

### dndscv - dichotomous heteroplasmy split
Broadly speaking, the threshold between positive and negative selection is ~20%.  Therefore, to maximise power to detect gene-level effects, run analysis with single cut-off at this threshold. This maximises the power to detect differences in other tissues (such as the non-blood tissues), but also to find any gene-specific differences.

```{r dnds_dichot_heteroplasmy_heatmap,message=FALSE,warning=FALSE}
...
dnds_heatmap_by_dichotomous_vaf_by_gene

save_plot(filename = paste0(plots_dir,"dnds_heatmap_by_dichotomous_vaf_by_gene.pdf"),dnds_heatmap_by_dichotomous_vaf_by_gene,width=2.2,height=2.5)

```

## Re-analysis of Ju et al, cancer data

```{r ju_VAF_distribution,fig.height=7,fig.width=7}
ju_dat<-readxl::read_excel(paste0(root_dir,"/data/elife-02935-supp2-v2.xlsx"))

ju_dat_reduced<-ju_dat%>%dplyr::select("sampleID"=Tumour_sampleID,
                                 "pos"=mtDNA_pos,
                                 "ref"=wt_allele,
                                 "alt"=var_allele,
                                 "vaf"=`VAF(T)`)%>%
  mutate("chr"="MT",.before="pos")

#Look at the heteroplasmy distribution
ju_vaf_distribution<-ju_dat_reduced%>%
  ggplot(aes(x=vaf))+
  geom_histogram(col="black",linewidth=0.2,fill="lightblue")+
  theme_classic()+
  my_markdown_theme+
  labs(x="Variant allele fraction")
ju_vaf_distribution
save_plot(filename = paste0(plots_dir,"ju_vaf_distribution.pdf"),ju_vaf_distribution,width=2,height=2.5)
```


```{r ju_global_dnds,fig.height=8.8,fig.width=7,message=FALSE,warning=FALSE}
dndsout<-dndscv(ju_dat_reduced,
       gene_list=all_mtDNA_genes, 
       refdb = mtref_rda_path,
       numcode = 2,
       max_coding_muts_per_sample = Inf,
...
get_dnds_pvalue <- function(globaldnds, term = "wmis") {
  mle    <- globaldnds[term, "mle"]
  cilow  <- globaldnds[term, "cilow"]
  cihigh <- globaldnds[term, "cihigh"]
  
  log_se <- (log(cihigh) - log(cilow)) / (2 * 1.96)
  z      <- log(mle) / log_se
  p      <- 2 * pnorm(-abs(z))
  
  return(data.frame(term = term, mle = mle, 
...
results <- do.call(rbind, lapply(
  c("wmis", "wnon", "wspl", "wtru", "wall"),
  function(t) get_dnds_pvalue(dndsout$globaldnds, t)
))

#Visualize results
max_dnds_value=1.6
stats_to_include=c("Missense","Truncating","Overall")
ju_global_dnds_plot<-dndsout$globaldnds%>%
  filter(!is.na(name) & complete.cases(.))%>%
  mutate(name=factor(rename_vec[name],levels=stats_to_include))%>%
  filter(name%in%stats_to_include)%>%
  ggplot(aes(x=name,y = mle, ymin = cilow, ymax = cihigh, color = name, shape = name)) +
  geom_linerange(position= position_dodge2(width=0.75), linewidth = 0.5, color="darkgrey") +
  geom_point(position=position_dodge2(width=0.75), size = 1.5)+
  ...
  theme(axis.title.x=element_blank(),
        ...
        legend.position="none")
ju_global_dnds_plot
save_plot(filename = paste0(plots_dir,"ju_global_dnds_plot.pdf"),ju_global_dnds_plot,width=2,height=2.5)
```


```{r ju_dnds_by_gene_heatmap,fig.height=8.8,fig.width=7}
temp<-dndsout$sel_cv%>%
  dplyr::select(gene_name,wmis_cv,wnon_cv,qmis_cv,qtrunc_cv)

qval_cutoff=0.05
ju_dnds_by_gene_heatmap<-dplyr::bind_rows(temp%>%dplyr::select(gene_name,"dNdS"=wmis_cv,"qval"=qmis_cv)%>%mutate(type="Missense"),
                 temp%>%dplyr::select(gene_name,"dNdS"=wnon_cv,"qval"=qtrunc_cv)%>%mutate(type="Nonsense"))%>%
  dplyr::filter(gene_name%in%target_genes)%>%
  mutate(dnds_if_signif=ifelse(qval<qval_cutoff,paste0(round(dNdS,2),"*"),""),
         gene_name=factor(gene_name,levels=rev(gene_order)))%>%
  ggplot(aes(y=gene_name,x=type,fill=pmin(dNdS,2),label=dnds_if_signif))+
  geom_tile()+
  geom_text(aes(col=type),size=2)+
  scale_color_manual(values=c("black","white"),guide="none")+
  theme_classic()+
  my_markdown_theme+
  ...
  scale_fill_gradientn(colors=RColorBrewer::brewer.pal(n=11,name = "RdYlGn"),limits=c(0,2))+
  labs(x="",y="",fill="dN/dS")
ju_dnds_by_gene_heatmap
save_plot(filename = paste0(plots_dir,"ju_dnds_by_gene_heatmap.pdf"),ju_dnds_by_gene_heatmap,width=2,height=2.5)
```

Analyse by heteroplasmy, as for the normal blood data

```{r ju_group_by_VAF,fig.height=4.4,fig.width=7,message=FALSE,warning=FALSE}
...
#Assign each observed VAF into its 'VAF group' bin
ju_dat_reduced$VAF_group=sapply(ju_dat_reduced$vaf,function(vaf) {
  if(vaf==0) {
    return("Absent")
  } else {
    return(VAF_groups$labels[VAF_groups$LL<vaf & VAF_groups$UL>=vaf])
...
dndscv_by_dichotomous_vaf_blood<-lapply(new_VAF_groups[2:length(new_VAF_groups)],function(VAF_bin) {
  cat(VAF_bin,sep="\n")
  tissue_vaf_info<-ju_dat_reduced%>%
    dplyr::filter(VAF_group==VAF_bin)%>%
    dplyr::filter(!duplicated(.))%>% #only count each mutation once per individual in each VAF group
    arrange(sampleID,pos)
...
globaldnds_res_by_vaf<-Map(dndsout=dndscv_by_dichotomous_vaf_blood,VAF_bin=new_VAF_groups[2:length(new_VAF_groups)],function(dndsout,VAF_bin) {
  dndsout$globaldnds%>%
    filter(!is.na(name) & complete.cases(.))%>%
    mutate(name=rename_vec[name])%>%
    mutate(VAF_group=VAF_bin)
...
stats_to_include="Overall"
stats_to_include=c("Missense","Truncating","Overall")
max_dnds_value=3
ju_global_dnds_missense_by_vaf_plot<-globaldnds_res_by_vaf%>%
  filter(VAF_group%in%VAF_groups$labels[-1] & name%in%stats_to_include)%>%
  mutate(cihigh=ifelse(cihigh>max_dnds_value,max_dnds_value,cihigh),
         VAF_group=factor(VAF_group,levels=VAF_groups$labels[-1]),
         name=factor(name,levels=stats_to_include))%>%
...
ju_global_dnds_missense_by_vaf_plot

save_plot(filename = paste0(plots_dir,"ju_global_dnds_missense_by_vaf_plot.pdf"),ju_global_dnds_missense_by_vaf_plot,width=4,height=2.5)
```


```{r ju_dnds_by_vaf_by_gene,fig.height=7,fig.width=7}
### Now at a per gene level
selcv_res_by_vaf<-Map(dndsout=dndscv_by_dichotomous_vaf_blood,VAF_bin=new_VAF_groups[2:length(new_VAF_groups)],function(dndsout,VAF_bin) {
  temp<-dndsout$sel_cv%>%
    dplyr::select(gene_name,wmis_cv,wnon_cv,qmis_cv,qtrunc_cv)
...
ju_dnds_heatmap_by_vaf_by_gene<-selcv_res_by_vaf%>%
  mutate(dnds_if_signif=ifelse(qval<qval_cutoff,paste0(round(dNdS,2),"*"),""),
         gene_name=factor(gene_name,levels=rev(gene_order)))%>%
  filter(VAF_group%in%VAF_groups$labels[-1])%>%
  dplyr::filter(type=="Missense")%>%
...
ju_dnds_heatmap_by_vaf_by_gene

save_plot(filename = paste0(plots_dir,"ju_dnds_heatmap_by_vaf_by_gene.pdf"),ju_dnds_heatmap_by_vaf_by_gene,width=2.5,height=2.5)
