...
option_list = list(
  ...
  make_option(c("-M", "--method"), action="store", default='neuralnet', type='character', help="abc() method: 'neuralnet' (regression-adjusted) or 'rejection' (unadjusted). The published Methods describe rejection; the surviving code used neuralnet."),
...
cohort_config<-list(
  normal=list(mito_data_file="data/mito_data.Rds",
              ...
              out_subdir="Drift_ABC_clonal_expansions/Drift_ABC_normal_sequential",
              out_subdir_individual="Drift_ABC_clonal_expansions/Drift_ABC_normal_individual"),
  MPN=list(mito_data_file="data/nonblood/mito_mutation_data_NW.RDS", #"NW" is the blood_MPN cohort
           ...
           out_subdir="Drift_ABC_clonal_expansions/Drift_ABC_MPN_sequential",
           out_subdir_individual="Drift_ABC_clonal_expansions/Drift_ABC_MPN_individual"),
  ...
  #rather than keyed by donor, but its ultratrees are identical to the copies
  #stored in the data object (verified by root-to-tip distance).
  CML=list(mito_data_file="data/nonblood/mito_mutation_data_CML.RDS",
           trees_file=NA,
           tree_slot="tree.ultra",
           units_per_year=1,            #CML tree.ultra is scaled to years (root-to-tip 10.4 and 59.7)
           absence_threshold=0.03,
           muts_file="abc_muts_CML.csv",
           out_subdir="Drift_ABC_clonal_expansions/Drift_ABC_CML_sequential",
           out_subdir_individual="Drift_ABC_clonal_expansions/Drift_ABC_CML_individual")
)

if(!opt$type%in%names(cohort_config)) {stop("--type must be one of: ",paste(names(cohort_config),collapse=", "))}
config<-cohort_config[[opt$type]]
if(!opt$abc_type%in%c("sequential","individual")) {stop("--abc_type must be 'sequential' or 'individual'")}
if(!opt$method%in%c("neuralnet","rejection")) {stop("--method must be 'neuralnet' or 'rejection'")}
cat("Cohort:",opt$type,"| ABC type:",opt$abc_type,"| method:",opt$method,"| units_per_year:",config$units_per_year,"| absence threshold:",config$absence_threshold,"\n")
...
out_subdir=if(opt$abc_type=="individual") config$out_subdir_individual else config$out_subdir
...
muts_arg<-if(nchar(opt$muts_df)) opt$muts_df else config$muts_file
muts_file<-if(file.exists(muts_arg)) muts_arg else paste0(root_dir,"/simulation_scripts_for_ABCs/",muts_arg)
...
#
# DEVIATION: the original set j<-opt$j_index and then immediately overwrote it with
...
j_to_run<-if(opt$j_index==0) 1:nrow(abc_muts) else opt$j_index
...
for(j in j_to_run) {
  ...
  # Prior
  #-----------------------------------------------------------------------------
  if(opt$abc_type=="individual"|j==1) {
  ...
  # Simulate
  #-----------------------------------------------------------------------------
  #The raw simulations are only needed to derive the summary statistics. If those
  #have already been computed and cached, skip loading (or re-running) the
  #simulations entirely - this makes re-running the abc() step with a different
  #--method essentially free for an individual ABC, where the prior does not
  #depend on the method.
  if(file.exists(mut_params_and_sumstats_file)) {
    cat("Summary statistics already cached - skipping the simulation step\n")
    sim_out_list<-NULL
  } else if(file.exists(mut_sim_file)){
  ...
  abc.nn<-if(opt$method=="neuralnet") {
    abc(target = as.numeric(sumstats.data),param = abc_params,
        sumstat = params_and_sumstats$sumstats,tol=0.05,
        transf = c("log","log"),method = "neuralnet")
  } else {
    #Plain rejection: no regression adjustment, so transf does not apply and the
    #posterior is returned in unadj.values only.
    abc(target = as.numeric(sumstats.data),param = abc_params,
        sumstat = params_and_sumstats$sumstats,tol=0.05,
        method = "rejection")
  }
  ...
  #-----------------------------------------------------------------------------
  #neuralnet returns regression-adjusted values; rejection returns only the raw accepted set
  post_values<-if(opt$method=="neuralnet") abc.nn$adj.values else abc.nn$unadj.values
  posterior_table<-data.frame(starting_vaf=post_values[,"starting_vaf"],
                              generation_time=post_values[,"generation_time"])%>%
