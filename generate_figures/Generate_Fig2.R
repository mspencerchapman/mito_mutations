...
#Empirical cumulative distribution of VAF by consequence class, with two-sample
#KS tests against the synonymous distribution. Note the caveat that a variant
#can be measured in several samples from the same individual, so the
#measurements are not fully independent.
syn_vals  <- complete.annotated.mutation.table%>%dplyr::filter(impact=="Synonymous")%>%pull(vaf)
mis_vals  <- complete.annotated.mutation.table%>%dplyr::filter(impact=="Missense")%>%pull(vaf)
nons_vals <- complete.annotated.mutation.table%>%dplyr::filter(impact=="Nonsense")%>%pull(vaf)

ks_syn_mis <- ks.test(syn_vals, mis_vals)
ks_syn_non <- ks.test(syn_vals, nons_vals)

#Locate the VAF at which the two ecdfs are furthest apart - the KS statistic -
#so the annotation can be drawn at that point.
find_ks_coords <- function(vals1, vals2) {
  ecdf1 <- ecdf(vals1); ecdf2 <- ecdf(vals2)
...
fmt_p <- function(p) {
  ifelse(p < 0.001, "p < 0.001",
         ifelse(p < 0.01, paste0("p = ", round(p, 3)), paste0("p = ", round(p, 2))))
}
...
mutation_type_ecdf_plot<-complete.annotated.mutation.table%>%
  ...
  annotate("segment", x = coords_mis$xmin, xend = coords_mis$xmax,
           y = coords_mis$y, yend = coords_mis$y,
           colour = "black", linewidth = 0.5, linetype = "dashed") +
  annotate("text", x = mean(c(coords_mis$xmin, coords_mis$xmax)), y = coords_mis$y,
           vjust = -0.5, size = 2, label = paste0("Syn vs Mis\n", fmt_p(ks_syn_mis$p.value))) +
  annotate("segment", x = coords_non$xmin, xend = coords_non$xmax,
           y = coords_non$y, yend = coords_non$y,
           colour = "black", linewidth = 0.5, linetype = "dashed") +
  annotate("text", x = mean(c(coords_non$xmin, coords_non$xmax)), y = coords_non$y,
           vjust = -0.5, size = 2, label = paste0("Syn vs Nons\n", fmt_p(ks_syn_non$p.value))) +
  scale_y_continuous(limits = c(0.01, 1), labels = scales::percent_format(accuracy = 1),
...
ggsave(filename = paste0(plots_dir,"Figure_02/Fig2a.mutation_type_ecdf_plot.pdf"),mutation_type_ecdf_plot,width=2.5,height=2.5)

#-----------------------------------------------------------------------------------#
## Generate FIG. 2B----
#-----------------------------------------------------------------------------------#

#dN/dS across all blood samples pooled. The lapply over a single dummy element
#keeps the output shape ("one dndscv per group") used by the panels below; the
#input is deliberately not split by tissue.
dndscv_by_tissue<-lapply("x",function(tissue.id) {
  ...
  mtdna.dndsout <- dndscv(tissue_info, gene_list=target_genes,
                          refdb = mtref_rda_path, numcode = 2,
...
ggsave(filename = paste0(plots_dir,"Figure_02/Fig2b.global_dnds_blood.pdf"),global_dnds_blood,width=1.3,height=2.5)
...
ggsave(filename = paste0(plots_dir,"Figure_02/Fig2c.dnds_heatmap_by_gene.pdf"),dnds_heatmap_by_gene,width=1.5,height=2.5)

#-----------------------------------------------------------------------------------#
### Not a Fig. 2 panel: VAF by consequence class as violins. Superseded by the
### ecdf in panel a, kept here as exploratory output.
#-----------------------------------------------------------------------------------#
...
dndscv_by_vaf_blood<-lapply(new_VAF_groups,function(VAF_bin) {
  cat(VAF_bin,sep="\n")
  #All blood pooled, as in the submitted panel - not restricted to adult blood.
...
ggsave(filename = paste0(plots_dir,"Figure_02/Fig2d.global_dnds_by_vaf_blood.pdf"),global_dnds_by_vaf_plot,width=1.6,height=2.5)

#-----------------------------------------------------------------------------------#
### Not a Fig. 2 panel: gene-level dN/dS by VAF bin, kept as exploratory output.
#-----------------------------------------------------------------------------------#
...
#Broadly the threshold between positive and negative selection is ~20%, so for
#the gene-level analysis the variants are split dichotomously at that value.
#This maximises the power to detect gene-specific differences.
...
ggsave(filename = paste0(plots_dir,"Figure_02/Fig2e.dnds_heatmap_by_dichotomous_vaf_by_gene.pdf"),dnds_heatmap_by_dichotomous_vaf_by_gene,width=2.2,height=2.5)

#-----------------------------------------------------------------------------------#
### Generate FIG. 2F ---------
#-----------------------------------------------------------------------------------#

#Split the samples of the older individuals by whether they sit in a clonal
#expansion or are singletons, cutting the tree at 100 mutations of molecular
#time. mito_data[5:8] are the older individuals; the younger ones have too few
#expansions for this comparison.
mut_vs_wt_list<-Map(list=mito_data[5:8],exp_ID=names(mito_data)[5:8],function(list,exp_ID) {
  cat(exp_ID,sep="\n")
  #Only n_samples is used below, to separate expanded clades from singletons.
  #The original script also joined the nuclear driver annotation here, but
  #nDNA_mats is not part of the deposited mito_data.Rds and those columns were
  #never read, so the join is omitted.
...
ggsave(filename = paste0(plots_dir,"Figure_02/Fig2f.global_dnds_by_expansion_status.pdf"),global_dnds_by_expansion_status_dichotomous_VAF_plot,width=1.7,height=2.5)
