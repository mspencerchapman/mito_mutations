...
include_dummy_foetal_samples=F
all_sum_of_vaf_summary_df=lapply(all_cohorts,function(this_dataset) {
  ...
  #Based on mutation burdens of the cord blood HSPC data, assumption is that mtDNA mutation burdens likely to be fairly similar in most tissues at birth
  if(sum(sum_of_vaf_summary$Age<5)==0 & include_dummy_foetal_samples) {
...
#MODEL 1: lmer with only the age:dataset interaction term
#This way you get the term for the blood data as well
lmer1<-lmerTest::lmer(mean~Age:dataset+(1|exp_ID),data=all_sum_of_vaf_summary_df)
...
ggsave(filename = paste0(plots_dir,"global_dnds_by_tissue_and_vaf_plot.pdf"),global_dnds_by_tissue_and_vaf_plot,width=7,height=2)
