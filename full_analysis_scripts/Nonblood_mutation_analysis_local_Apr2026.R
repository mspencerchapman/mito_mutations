...
#Mean coverage - histograms as facets
...
#Print non-blood coverage summary statistics
#Per individual
mito_cn%>%
  filter(Tissue%in%translate$al_ref)%>%
  right_join(sample_summary_info,by=c("Sample"="SampleID"))%>%
  ...
  summarise(mean_mtDNA_coverage=mean(bedtools_mtDNA_coverage),prop_over_1000=sum(bedtools_mtDNA_coverage>1000)/n())
...
#This is already in the saved version, and this takes a while so don't need to re-run unless
#you want to change some parameters.
...
#Box plots of copy number by tissue type
...
#Ridges plots of copy number distribution across the tissue
...
#Dot plot of median copy number against age, points coloured by tissue plot
#There are overlying regression lines to show the relationship against age
...
#Mean copy number per tissue
#Given the log-normal distribution, perform the means on the log-transformed data
#then transform back to absolute numbers
...
#Create a dataframe to use with the linear mixed effects model
#Given that CN is a log-normal distribution, do the model against log CN
...
#LMER MODEL 2: Given no convincing age relationship, do LMER of logCN against tissue only
...
confint(mito_cn.lmer_lung_smoking)

#Visualize the LMER MODEL 2 tissue-specific coefficients
...
#Visualize the LMER MODEL 2 tissue-specific coefficients - now transformed back to absolute scale
intercept_value<-summary(mito_cn.lmer)$coefficients["(Intercept)","Estimate"]
...
dont_show_medians_tissues<-c("KY","LM","SO") #Too many individuals to plot medians (gets too busy)
temp=lapply(all_cohorts,function(this_dataset) {
  ...
  if(this_dataset%in%dont_show_medians_tissues) {
...
#-----------------------------------------------------------------------------------#
##----------ASSESSMENT OF MUTATION RATES ~ TISSUE ----------
#-----------------------------------------------------------------------------------#
...
all_samples_to_drop<-lapply(all_mito_datasets[all_cohorts],function(dataset_mito_data) {
  lapply(names(dataset_mito_data),function(this_exp_ID) {
    ...
    samples_to_drop=unlist(lapply(expanded_clade_nodes$nodes,function(node) {
      ...
      to_drop=sample(clade_samples,size=length(clade_samples)-1) #Drop all but one samples from a clade
    }))
  })
})%>%unlist()


# DO LMER MODELS ACROSS DATA-----
#Prepare the data, excluding individuals with very few samples
...
unique(all_sum_of_vaf_summary_df$Tissue_type)
all_sum_of_vaf_summary_df$tissue_cat<-ifelse(grepl("Colon",all_sum_of_vaf_summary_df$Tissue_type),"Colon",
                                             ifelse(grepl("Endometrium",all_sum_of_vaf_summary_df$Tissue_type),"Endometrium",
                                                    ifelse(grepl("Bronchial",all_sum_of_vaf_summary_df$Tissue_type),"Bronchial",
                                                           "Haematopoietic")))

# Get models of increasing complexity
#MODEL 1: Fully null model, constant burden with age
lmer.null<-lmerTest::lmer(mean~1+(1|exp_ID),data=all_sum_of_vaf_summary_df)

#MODEL 1: lmer with only an Age term i.e. a null model for which mutation rate cannot vary between tissues
lmer1<-lmerTest::lmer(mean~Age+(1|exp_ID),data=all_sum_of_vaf_summary_df)

#MODEL 2: lmer with only the age:dataset interaction term
#This way you get the term for the blood data as well
lmer2<-lmerTest::lmer(mean~Age:dataset+(1|exp_ID),data=all_sum_of_vaf_summary_df)

#MODEL 3: lmer with full set of Age*dataset terms
lmer3<-lmerTest::lmer(mean~Age*dataset+(1|exp_ID),data=all_sum_of_vaf_summary_df)

#Compare MODELS using the AIC
AIC(lmer.null,lmer1,lmer2, lmer3)
diff(AIC(lmer.null,lmer1,lmer2, lmer3)$AIC)

lmer2.tissue_cat<-lmerTest::lmer(mean~Age:tissue_cat+(1|exp_ID),data=all_sum_of_vaf_summary_df)
lmer3.tissue_cat<-lmerTest::lmer(mean~Age*tissue_cat+(1|exp_ID),data=all_sum_of_vaf_summary_df)

AIC(lmer.null,lmer1,lmer2.tissue_cat, lmer3.tissue_cat)
diff(AIC(lmer.null,lmer1,lmer2.tissue_cat, lmer3.tissue_cat)$AIC)
...
p1.1<-lmer_info_for_plotting%>%
  ...
  theme_classic()+
  scale_x_continuous(limits=c(0,0.017))+
  my_theme+
  theme(axis.title.y=element_blank(),legend.position = "none")+
  labs(x="Average mutations per mtDNA\nmolecule per cell per year")



## Tissue level plotting of lmer----

# Create prediction dataframe
pred_df <- expand.grid(
  Age = seq(min(all_sum_of_vaf_summary_df$Age),
            max(all_sum_of_vaf_summary_df$Age),
            length.out = 100),
  dataset = names(name_conversion_vec)
)

# Get predictions - re.form = NA excludes random effects
# giving population-level predictions
pred_df$fit <- predict(lmer2, newdata = pred_df, re.form = NA)

library(performance)
performance::check_model(lmer2)  # includes residuals vs fitted for heteroscedasticity
check_heteroscedasticity(lmer2)

# Confidence intervals via bootstrap
boot_ci <- lme4::bootMer(lmer2,
                   FUN = function(x) predict(x, newdata = pred_df, re.form = NA),
                   nsim = 1000,
                   use.u = F,
                   type = "parametric")
...
boot_ci <- apply(boot_coef$t, 2, quantile, c(0.025, 0.975))
t(boot_ci)

mutburden_by_age_and_tissue_plot<-ggplot() +
  geom_ribbon(data = pred_df%>%mutate(dataset=factor(name_conversion_vec[dataset],name_conversion_vec)),
              aes(x = Age, ymin = lower, ymax = upper,
                  fill = dataset), alpha = 0.1) +
  geom_line(data = pred_df%>%mutate(dataset=factor(name_conversion_vec[dataset],name_conversion_vec)),
            aes(x = Age, y = fit, colour = dataset)) +
  geom_point(data = all_sum_of_vaf_summary_df%>%mutate(dataset=factor(name_conversion_vec[dataset],name_conversion_vec)),
             aes(x = Age, y = mean, colour = dataset),
             alpha = 0.6) +
...
ggsave(filename = paste0(plots_dir,"mutburden_by_age_and_tissue_plot.pdf"),mutburden_by_age_and_tissue_plot,width=7,height=2)


# Bootstrap the model coefficients directly
boot_coef <- lme4::bootMer(lmer2,
                     FUN = lme4::fixef,
                     nsim = 1000,
                     use.u = FALSE,
                     type = "parametric")

# Extract CIs for each slope
boot_ci <- apply(boot_coef$t, 2, quantile, c(0.025,0.975))
t(boot_ci)


lmer_info_for_plotting<-as.data.frame(t(boot_ci))%>%
  dplyr::bind_cols(summary(lmer2)$coefficients)%>%
  tibble::rownames_to_column(var="name")%>%
  left_join(as.data.frame(CI_table)%>%tibble::rownames_to_column(var="name"),by="name")%>%
  filter(grepl("dataset",name))%>%
  mutate(name=gsub("dataset","",name))%>%
...
ggsave(filename=paste0(plots_dir,"lmer_terms.pdf"),p1.1,width=2.5,height=2)
...
ggsave(filename = paste0(plots_dir,"consequence_change_plot_CROSSTISSUE.pdf"),consequence_change_plot,width=3.5,height=2.5)

impact_comparison_df%>%
  filter(impact!="Non-Coding" & tissue!="blood")%>%
  mutate(change=paste(impact_NUC,impact,sep=">"))%>%
  mutate(altered=ifelse(impact_NUC==impact,"unchanged","changed"))%>%
  pull(altered)%>%table()
...
ggsave(filename = paste0(plots_dir,"total_numbers_old_vs_new_plot_CROSSTISSUE.pdf"),total_numbers_old_vs_new_plot,width=3,height=2.5)
...
ggsave(filename = paste0(rebuttal_figs_dir,"mutation_type_by_gene_CROSSTISSUE.pdf"),mutation_type_by_gene,width=3.5,height=2.5)
...
ggsave(filename = paste0(rebuttal_figs_dir,"mutation_type_by_tissue_CROSSTISSUE.pdf"),mutation_type_by_tissue,width=3.5,height=2)
...
sapply(list(syn_vals,mis_vals,nons_vals),length)
...
ggsave(filename = paste0(rebuttal_figs_dir,"mutation_type_ecdf_plot_EPITHELIAL.pdf"),mutation_type_ecdf_plot,width=2.5,height=2)
...
ggsave(filename = paste0(plots_dir,"dnds_global_by_dichotomous_vaf_epithelial_plot.pdf"),dnds_global_by_dichotomous_vaf_epithelial_plot,width=3,height=2)

  
selcv_res_by_tissue_and_vaf<-Map(dndsout=dndsout.epithelial,VAF_bin=c("<20%",">20%"),function(dndsout,VAF_bin) {
    temp<-dndsout$sel_cv%>%
    ...
    mutate(VAF_group=factor(VAF_group,levels=c("<20%",">20%")))

qval_cutoff=0.05
dnds_heatmap_epithelial_and_vaf_by_gene<-selcv_res_by_tissue_and_vaf%>%
...
ggsave(filename = paste0(plots_dir,"dnds_heatmap_epithelial_and_vaf_by_gene.pdf"),dnds_heatmap_epithelial_and_vaf_by_gene,width=2.5,height=2)
...
ggsave(filename = paste0(plots_dir,"global_dnds_CROSSTISSUE.pdf"),global_dnds_nonblood,width=2,height=2)
...
ggsave(filename = paste0(plots_dir,"global_dnds_by_tissue_and_vaf_plot_CROSSTISSUE.pdf"),global_dnds_by_tissue_and_vaf_plot,width=7,height=2)
...
#Split into two groups so that can fit in the figure
## GROUP 1
...
ggsave(filename = paste0(plots_dir,"dnds_heatmap_by_tissue_and_vaf_by_gene_group1.pdf"),dnds_heatmap_by_tissue_and_vaf_by_gene_group1,width=3.5,height=4)
ggsave(filename = paste0(plots_dir,"dnds_heatmap_by_tissue_and_vaf_by_gene_group2.pdf"),dnds_heatmap_by_tissue_and_vaf_by_gene_group2,width=3.5,height=3.2)
...
#-----------------------------------------------------------------------------------#
# -------------Split by dichotomous VAF group (less or greater than 20% only) -----
#-----------------------------------------------------------------------------------#
...
dndscv_by_tissue_and_dichotomous_vaf<-Map(df_tidy=all_df_tidy[all_cohorts],this_tissue=all_cohorts,function(df_tidy,this_tissue) {
  cat(this_tissue,sep = "\n")
  dndscv_by_vaf_tissue<-lapply(new_VAF_groups[2:length(new_VAF_groups)],function(VAF_bin) {
    cat(VAF_bin,sep="\n")
    tissue_vaf_info<-df_tidy%>%
...
globaldnds_res_by_tissue_and_dichotomous_vaf<-Map(list1=dndscv_by_tissue_and_dichotomous_vaf,this_tissue=all_cohorts,function(list1,this_tissue) {
  globaldnds_res_by_vaf<-Map(dndsout=list1,VAF_bin=new_VAF_groups[2:length(new_VAF_groups)],function(dndsout,VAF_bin) {
    dndsout$globaldnds%>%
      filter(!is.na(name) & complete.cases(.))%>%
      mutate(name=rename_vec[name])%>%
...
max_dnds_value<-3
stats_to_include=c("Overall")
VAF_conv_vec=c("<20%",">20%")
names(VAF_conv_vec)=c("3-20%","20-100%")
global_dnds_by_tissue_and_dichotomous_vaf_plot<-globaldnds_res_by_tissue_and_dichotomous_vaf%>%
  filter(VAF_group%in%VAF_groups$labels[2:length(boundaries)] & name%in%stats_to_include)%>%
  mutate(cihigh=ifelse(cihigh>max_dnds_value,max_dnds_value,cihigh),
         VAF_group=factor(VAF_conv_vec[VAF_group],levels=VAF_conv_vec),
         tissue=factor(name_conversion_vec[tissue],levels=name_conversion_vec))%>%
  ggplot(aes(x=VAF_group, y = mle, ymin = cilow, ymax = cihigh, color = name, shape = name)) +
  geom_hline(yintercept=1, linetype='dashed', col = 'black', linewidth = 0.5) +
  geom_linerange(position= position_dodge2(width=0.75), linewidth = 0.5, color="darkgrey") +
...
ggsave(filename = paste0(plots_dir,"global_dnds_by_tissue_and_dichotomous_vaf_plot_CROSSTISSUE.pdf"),global_dnds_by_tissue_and_dichotomous_vaf_plot,width=2.5,height=2.5)
...
# are most likely to have been present in the oocyte and drifted up in unrelated lineages.
# The alternative - possible for some of these - are that these are examples of independent acquisition.
