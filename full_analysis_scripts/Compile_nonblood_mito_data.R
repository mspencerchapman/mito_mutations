...
temp=lapply(c("SO","LM","HL","KY","PR","NW"),function(dataset) {
  #dataset="SO"
  ...
  #Deal with the germline mutations
  ...
  #Identify likely heteroplasmic oocyte mutations - for most purposes these are excluded from mutation burden estimates as they don't reflect new somatic mutation/ drift
  dataset_mito_data<-Map(list=dataset_mito_data,exp_ID=names(dataset_mito_data),function(list,exp_ID) {
    cat(exp_ID,sep="\n")
    if(is.null(list$het_oocyte_muts)) {
      list$het_oocyte_muts<-detect_het_oocyte_mutation(matrices=list$matrices,tree=list$tree,vaf_cutoff=0.3)
    }
    return(list)
  })
  ...
  temp=Map(list=dataset_mito_data,exp_ID=names(dataset_mito_data),f=function(list,exp_ID){
    ...
    if(is.null(list$shared_muts_df)) {
      ...
      plot_tree(tree = list[[tree_to_show]],cex.label = 0,plot_axis=F,vspace.reserve = 1.1,title=paste0(exp_ID,": Shared mutations (VAF>",100*lowest_VAF_to_show,"%)"))
      dev.off()
      stop(return(NULL))
    }
    ...
    par(mfrow=c(1,1))
    pdf(file = paste0(figures_dir,exp_ID,"_shared_muts.pdf"),width = 7,height=4)
    list[[tree_to_show]]=plot_tree(tree = list[[tree_to_show]],cex.label = 0,plot_axis=T,vspace.reserve = 1.1,title=paste0(exp_ID,": Shared mutations (VAF>",100*lowest_VAF_to_show,"%)"))
    add_mito_mut_heatmap(tree=list[[tree_to_show]],heatmap=hm[plot_muts.clustered$order,,drop=F],border="gray",heatmap_bar_height=0.15,cex.label = 0.25)
    dev.off()
  })
  ...
  temp=Map(list=dataset_mito_data,exp_ID=names(dataset_mito_data),f=function(list,exp_ID){
    ...
    if(is.null(list$shared_muts_df)) {
      ...
      list$tree=plot_tree(tree = list$tree,cex.label = 0,plot_axis=T,vspace.reserve = 1.1,title=paste0(exp_ID,": Shared mutations (VAF>",100*lowest_VAF_to_show,"%)"))
      dev.off()
      stop(return(NULL))
    }
    if(nrow(list$shared_muts_df)==0) {
    ...
    pdf(file = paste0(figures_dir,exp_ID,"_raw_shared_muts.pdf"),width = 7,height=4)
    list$tree=plot_tree(tree = list$tree,cex.label = 0,plot_axis=F,vspace.reserve = 1.1,title=paste0(exp_ID,": Shared mutations (VAF>",100*lowest_VAF_to_show,"%)"))
    add_mito_mut_heatmap(tree=list$tree,heatmap=hm[plot_muts.clustered$order,,drop=F],border="gray",heatmap_bar_height=0.08,cex.label = 0.25)
    dev.off()
  })
  ...
  temp=Map(list=dataset_mito_data,exp_ID=names(dataset_mito_data),f=function(list,exp_ID){
    ...
    if(length(plot_muts)==0) {
      pdf(file = paste0(figures_dir,exp_ID,"_raw_shared_muts.pdf"),width = 7,height=4)
      list$tree=plot_tree(tree = list$tree,cex.label = 0,plot_axis=T,vspace.reserve = 1.1,title=paste0(exp_ID,": All mutations (VAF>",100*lowest_VAF_to_show,"%)"))
      dev.off()
      stop(return(NULL))
    }
    ...
    list$tree=plot_tree(tree = list$tree,cex.label = 0,plot_axis=T,vspace.reserve = 4,title=paste0(exp_ID,": All mutations (VAF>",100*lowest_VAF_to_show,"%)"))
...
expanded.clades.plot<-dplyr::bind_rows(Map(list=dataset_mito_data,exp_ID=names(dataset_mito_data),function(list,exp_ID){
  exp_nodes<-get_expanded_clade_nodes(list$tree.ultra,height_cut_off = ifelse(dataset=="NW",50,200),min_clonal_fraction=0.01,min_samples = 2)
