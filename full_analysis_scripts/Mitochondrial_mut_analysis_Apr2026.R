...
#-----------------------------------------------------------------------------------#
##----------------------Gene-level dNdS info by dichotomous VAF division (20%) -----
#-----------------------------------------------------------------------------------#

#Broadly speaking, the threshold between positive and negative selection is ~20%
#Therefore, to maximise power to detect gene-level effects, run analysis with single cut-off at this threshold
...
#-----------------------------------------------------------------------------------#
##----------------------Complex-level dNdS info by dichotomous VAF division (20%) -----
#-----------------------------------------------------------------------------------#

complexes=list(ComplexI=c("MT-ND5", "MT-ND2", "MT-ND3","MT-ND4", "MT-ND1","MT-ND4L"),
               ComplexIII="MT-CYB",
               ComplexIV=c("MT-CO1","MT-CO2","MT-CO3"),
               ComplexV=c("MT-ATP6","MT-ATP8"))

dndscv_by_vaf_by_complex_blood<-lapply(new_VAF_groups,function(VAF_bin) {
  cat(VAF_bin,sep="\n")
  tissue_vaf_info<-df_tidy%>%
    dplyr::filter(VAF_group==VAF_bin)%>%
    separate("mut_ref", c("chr", "pos", "ref", "mut"), "_")%>%
  ...
  dnds_by_complex<-Map(complex_genes=complexes,complex_names=names(complexes),function(complex_genes,complex_names) {
    cat(complex_genes,sep="\n")
    mtdna.dndsout <- dndscv(tissue_vaf_info,
                            gene_list=complex_genes, 
                            refdb = mtref_rda_path,
                            max_coding_muts_per_sample = Inf,
                            numcode = 2,
                            max_muts_per_gene_per_sample = Inf,
  ...
  return(dnds_by_complex)
})

dnds_by_VAF_and_complex<-Map(dndsout_by_complex=dndscv_by_vaf_by_complex_blood,VAF_bin=new_VAF_groups,function(dndsout_by_complex,VAF_bin) {
  
  Map(dndsout=dndsout_by_complex,complex=names(complexes),function(dndsout,complex) {
    dndsout$globaldnds%>%
      filter(!is.na(name) & complete.cases(.))%>%
      mutate(name=rename_vec[name])%>%
      mutate(VAF_group=VAF_bin,Complex=complex)
  })%>%bind_rows()
  
})%>%bind_rows()

dnds_by_VAF_and_complex%>%
  mutate(dnds_if_signif=ifelse(cilow>1|cihigh<1,paste0(round(mle,2),"*"),""),
         Complex=factor(Complex,levels=names(complexes)))%>%
  filter(VAF_group%in%VAF_groups$labels[2:length(boundaries)])%>%
  dplyr::filter(name=="Missense")%>%
  ggplot(aes(y=Complex,x=VAF_group,fill=pmin(mle,2),label=round(mle,2)))+
  geom_tile()+
  facet_grid(~name)+
  geom_text(aes(col=name),size=1.6)+
  scale_color_manual(values=c("black","white"),guide="none")+
  theme_classic()+
  my_theme+
  #theme(axis.text.x=element_text(angle=90))+
...
global_dnds_young_or_old_plot<-global_dnds_by_young_or_old_status%>%
  ...
  geom_linerange(position= position_dodge2(width=0.75), size = 0.5, color="darkgrey") +
  geom_point(position=position_dodge2(width=0.75), size = 1.5) +
  labs(y = "Mitochondrial genome-wide dN/dS", x = "Tissue") +
  theme_classic() + 
  ylim(c(0,max_dnds_value))+
  scale_x_discrete(labels = function(x) str_wrap(x, width = 6))+
  scale_color_manual(values = mut_type_cols, name="Mutation type") +
...
### FOR THESE ANALYSES HAVE A DICHOTOMOUS VAF SPLIT
boundaries<-c(0,0.01,0.2,1)
new_VAF_groups=c("<1%","<20%",">20%")
VAF_groups=data.frame(labels=new_VAF_groups,LL=head(boundaries,-1),UL=tail(boundaries,-1))
...
max_dnds_value<-2.1
stats_to_include=c("Overall")
rename_mut_status_vec=c("DTA mutation","Wild-type")
names(rename_mut_status_vec)<-c("mut","wt")

global_dnds_by_mut_status_dichotomous_VAF_plot<-global_dnds_by_mut_status%>%
  filter(VAF_group%in%VAF_groups$labels[2:length(boundaries)] & name%in%stats_to_include)%>%
  mutate(cihigh=ifelse(cihigh>max_dnds_value,max_dnds_value,cihigh),
         VAF_group=factor(VAF_group,levels=VAF_groups$labels[2:length(boundaries)]),
...
ggsave(filename = paste0(rebuttal_figs_dir,"global_dnds_by_mut_status_dichotomous_VAF_plot.pdf"),global_dnds_by_mut_status_dichotomous_VAF_plot,width=2,height=2)
...
max_dnds_value<-1.7
stats_to_include=c("Overall")
rename_mut_status_vec=c("Clonal\nexpansion","Singleton")
names(rename_mut_status_vec)<-c("mut","wt")
...
ggsave(filename = paste0(rebuttal_figs_dir,"global_dnds_by_expansion_status_dichotomous_VAF_plot.pdf"),global_dnds_by_expansion_status_dichotomous_VAF_plot,width=1.7,height=2.5)
...
dnds_heatmap_by_expansion_status_dichotomous_VAF_by_gene<-selcv_res_by_expansion_status%>%
  mutate(dnds_if_signif=ifelse(qval<qval_cutoff,paste0(sprintf(dNdS, fmt = '%#.1f'),"*"),""),
         dNdS=ifelse(dNdS>2,2,dNdS),
         gene_name=factor(gene_name,levels=rev(gene_order)),
         mut_status=rename_mut_status_vec[mut_status])%>%
  filter(VAF_group%in%VAF_groups$labels[2:length(boundaries)])%>%
  dplyr::filter(type%in%stats_to_include)%>%
...
ggsave(filename = paste0(rebuttal_figs_dir,"dnds_heatmap_by_expansion_status_dichotomous_VAF_by_gene.pdf"),dnds_heatmap_by_expansion_status_dichotomous_VAF_by_gene,width=2.5,height=3.5)
